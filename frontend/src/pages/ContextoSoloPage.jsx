...
export function ContextoSoloPage({ setView }) {
  ...
  const saveContextoMatch = async (word, isWin, guessCount, thumbnail) => {
    if (hasSavedMatchRef.current) return;
    hasSavedMatchRef.current = true;

    const finalScore = isWin ? Math.max(25, 100 - Math.max(0, guessCount - 1) * 3) : 0;
    const roundRecord = {
      prompt: word ? `${word} (${guessCount} ${guessCount === 1 ? 'guess' : 'guesses'})` : 'Contexto Solo',
      difficulty: difficulty || 'medium',
      score: finalScore,
      doodle_image: thumbnail || ''
    };

    try {
      await api.saveGame([roundRecord], finalScore, 'contexto');
      showToast('Contexto game saved to your match history!', 'success', 3000);
    } catch (err) {
      console.warn('Could not persist Contexto session to history:', err);
    }
  };
  ...
  const handleGuess = async () => {
    ...
    try {
      ...
      if (res.is_match) {
        ...
        saveContextoMatch(res.secret_word, true, res.guess_count || guesses.length + 1, base64);
      } else {
  ...
  const handleConfirmGiveUp = async () => {
    ...
    try {
      ...
      const latestThumbnail = guesses[0]?.thumbnail || (canvasRef.current ? canvasRef.current.getImageBase64() : '');
      saveContextoMatch(res.secret_word, false, guesses.length, latestThumbnail);
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col h-[calc(100vh-5rem)]">
      ...
      <ConfirmModal
        ...
        message="Do you want to leave Contexto Solo and return to the dashboard? Your active guessing progress and stopwatch will be reset."
        confirmText="Leave"
