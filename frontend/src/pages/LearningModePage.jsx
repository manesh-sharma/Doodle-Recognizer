...
export function LearningModePage({ setView }) {
  ...
  // Selected level state (0-indexed, corresponding to level 1 to 197)
  // Automatically opens the first incomplete level (e.g. Level 4 if 1, 2, 3 completed); starts at Level 1 if all are complete
  const [currentLevelIndex, setCurrentLevelIndex] = useState(() => {
    try {
      const saved = localStorage.getItem('doodle_learning_progress');
      const savedProgress = saved ? JSON.parse(saved) : {};
      const firstIncompleteIdx = lessons.findIndex((l) => !savedProgress[l.level]);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
      ...
      <ConfirmModal
        ...
        message="Do you want to leave Learning Mode and return to the dashboard?"
        confirmText="Leave"
