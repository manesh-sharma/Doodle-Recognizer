// Comprehensive Doodle Blueprints & Learning Lessons Dataset
// Contains all curated active classes sorted Easy -> Medium -> Hard
// Every single active class provides authentic vector outline blueprints and tips.

export const ACTIVE_CLASSES_ORDERED = [
  {
    "id": 0,
    "name": "The Eiffel Tower",
    "class_name": "The Eiffel Tower",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 2,
    "name": "The Mona Lisa",
    "class_name": "The Mona Lisa",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 5,
    "name": "alarm clock",
    "class_name": "alarm clock",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 10,
    "name": "anvil",
    "class_name": "anvil",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 11,
    "name": "apple",
    "class_name": "apple",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 14,
    "name": "axe",
    "class_name": "axe",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 21,
    "name": "basket",
    "class_name": "basket",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 38,
    "name": "book",
    "class_name": "book",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 41,
    "name": "bowtie",
    "class_name": "bowtie",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 48,
    "name": "bucket",
    "class_name": "bucket",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 53,
    "name": "cactus",
    "class_name": "cactus",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 54,
    "name": "cake",
    "class_name": "cake",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 55,
    "name": "calculator",
    "class_name": "calculator",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 58,
    "name": "camera",
    "class_name": "camera",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 60,
    "name": "campfire",
    "class_name": "campfire",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 64,
    "name": "car",
    "class_name": "car",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 66,
    "name": "castle",
    "class_name": "castle",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 69,
    "name": "cell phone",
    "class_name": "cell phone",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 71,
    "name": "chair",
    "class_name": "chair",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 74,
    "name": "circle",
    "class_name": "circle",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 77,
    "name": "cloud",
    "class_name": "cloud",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 88,
    "name": "crown",
    "class_name": "crown",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 90,
    "name": "cup",
    "class_name": "cup",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 91,
    "name": "diamond",
    "class_name": "diamond",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 97,
    "name": "door",
    "class_name": "door",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 101,
    "name": "drums",
    "class_name": "drums",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 107,
    "name": "envelope",
    "class_name": "envelope",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 108,
    "name": "eraser",
    "class_name": "eraser",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 109,
    "name": "eye",
    "class_name": "eye",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 116,
    "name": "fire hydrant",
    "class_name": "fire hydrant",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 117,
    "name": "fireplace",
    "class_name": "fireplace",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 121,
    "name": "flashlight",
    "class_name": "flashlight",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 122,
    "name": "flip flops",
    "class_name": "flip flops",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 123,
    "name": "floor lamp",
    "class_name": "floor lamp",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 127,
    "name": "fork",
    "class_name": "fork",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 129,
    "name": "frying pan",
    "class_name": "frying pan",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 136,
    "name": "grass",
    "class_name": "grass",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 141,
    "name": "harp",
    "class_name": "harp",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 142,
    "name": "hat",
    "class_name": "hat",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 143,
    "name": "headphones",
    "class_name": "headphones",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 149,
    "name": "hockey stick",
    "class_name": "hockey stick",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 155,
    "name": "hourglass",
    "class_name": "hourglass",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 156,
    "name": "house",
    "class_name": "house",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 157,
    "name": "house plant",
    "class_name": "house plant",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 158,
    "name": "hurricane",
    "class_name": "hurricane",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 159,
    "name": "ice cream",
    "class_name": "ice cream",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 160,
    "name": "jacket",
    "class_name": "jacket",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 161,
    "name": "jail",
    "class_name": "jail",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 167,
    "name": "ladder",
    "class_name": "ladder",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 172,
    "name": "light bulb",
    "class_name": "light bulb",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 175,
    "name": "lightning",
    "class_name": "lightning",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 188,
    "name": "microwave",
    "class_name": "microwave",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 200,
    "name": "necklace",
    "class_name": "necklace",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 205,
    "name": "onion",
    "class_name": "onion",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 210,
    "name": "palm tree",
    "class_name": "palm tree",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 212,
    "name": "pants",
    "class_name": "pants",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 220,
    "name": "pencil",
    "class_name": "pencil",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 224,
    "name": "picture frame",
    "class_name": "picture frame",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 240,
    "name": "radio",
    "class_name": "radio",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 241,
    "name": "rain",
    "class_name": "rain",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 242,
    "name": "rainbow",
    "class_name": "rainbow",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 249,
    "name": "rollerskates",
    "class_name": "rollerskates",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 250,
    "name": "sailboat",
    "class_name": "sailboat",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 255,
    "name": "scissors",
    "class_name": "scissors",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 259,
    "name": "see saw",
    "class_name": "see saw",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 267,
    "name": "skull",
    "class_name": "skull",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 270,
    "name": "smiley face",
    "class_name": "smiley face",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 271,
    "name": "snail",
    "class_name": "snail",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 274,
    "name": "snowflake",
    "class_name": "snowflake",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 275,
    "name": "snowman",
    "class_name": "snowman",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 281,
    "name": "spreadsheet",
    "class_name": "spreadsheet",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 285,
    "name": "stairs",
    "class_name": "stairs",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 286,
    "name": "star",
    "class_name": "star",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 298,
    "name": "sun",
    "class_name": "sun",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 301,
    "name": "swing set",
    "class_name": "swing set",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 302,
    "name": "sword",
    "class_name": "sword",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 309,
    "name": "television",
    "class_name": "television",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 316,
    "name": "tooth",
    "class_name": "tooth",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 323,
    "name": "tree",
    "class_name": "tree",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 328,
    "name": "umbrella",
    "class_name": "umbrella",
    "difficulty": "easy",
    "label": "easy"
  },
  {
    "id": 6,
    "name": "ambulance",
    "class_name": "ambulance",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 7,
    "name": "angel",
    "class_name": "angel",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 15,
    "name": "backpack",
    "class_name": "backpack",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 19,
    "name": "baseball",
    "class_name": "baseball",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 22,
    "name": "basketball",
    "class_name": "basketball",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 24,
    "name": "bathtub",
    "class_name": "bathtub",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 28,
    "name": "bed",
    "class_name": "bed",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 32,
    "name": "bicycle",
    "class_name": "bicycle",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 33,
    "name": "binoculars",
    "class_name": "binoculars",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 44,
    "name": "bread",
    "class_name": "bread",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 50,
    "name": "bus",
    "class_name": "bus",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 51,
    "name": "bush",
    "class_name": "bush",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 52,
    "name": "butterfly",
    "class_name": "butterfly",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 61,
    "name": "candle",
    "class_name": "candle",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 65,
    "name": "carrot",
    "class_name": "carrot",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 73,
    "name": "church",
    "class_name": "church",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 79,
    "name": "compass",
    "class_name": "compass",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 81,
    "name": "cookie",
    "class_name": "cookie",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 89,
    "name": "cruise ship",
    "class_name": "cruise ship",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 99,
    "name": "dresser",
    "class_name": "dresser",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 103,
    "name": "dumbbell",
    "class_name": "dumbbell",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 104,
    "name": "ear",
    "class_name": "ear",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 110,
    "name": "eyeglasses",
    "class_name": "eyeglasses",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 119,
    "name": "fish",
    "class_name": "fish",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 124,
    "name": "flower",
    "class_name": "flower",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 125,
    "name": "flying saucer",
    "class_name": "flying saucer",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 126,
    "name": "foot",
    "class_name": "foot",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 132,
    "name": "giraffe",
    "class_name": "giraffe",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 137,
    "name": "guitar",
    "class_name": "guitar",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 138,
    "name": "hamburger",
    "class_name": "hamburger",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 139,
    "name": "hammer",
    "class_name": "hammer",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 140,
    "name": "hand",
    "class_name": "hand",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 145,
    "name": "helicopter",
    "class_name": "helicopter",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 148,
    "name": "hockey puck",
    "class_name": "hockey puck",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 151,
    "name": "hospital",
    "class_name": "hospital",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 163,
    "name": "key",
    "class_name": "key",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 165,
    "name": "knee",
    "class_name": "knee",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 168,
    "name": "lantern",
    "class_name": "lantern",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 169,
    "name": "laptop",
    "class_name": "laptop",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 170,
    "name": "leaf",
    "class_name": "leaf",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 173,
    "name": "lighter",
    "class_name": "lighter",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 180,
    "name": "lollipop",
    "class_name": "lollipop",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 181,
    "name": "mailbox",
    "class_name": "mailbox",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 187,
    "name": "microphone",
    "class_name": "microphone",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 190,
    "name": "moon",
    "class_name": "moon",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 194,
    "name": "mouse",
    "class_name": "mouse",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 196,
    "name": "mouth",
    "class_name": "mouth",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 198,
    "name": "mushroom",
    "class_name": "mushroom",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 213,
    "name": "paper clip",
    "class_name": "paper clip",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 214,
    "name": "parachute",
    "class_name": "parachute",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 217,
    "name": "peanut",
    "class_name": "peanut",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 218,
    "name": "pear",
    "class_name": "pear",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 219,
    "name": "peas",
    "class_name": "peas",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 227,
    "name": "pineapple",
    "class_name": "pineapple",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 228,
    "name": "pizza",
    "class_name": "pizza",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 231,
    "name": "pond",
    "class_name": "pond",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 233,
    "name": "popsicle",
    "class_name": "popsicle",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 236,
    "name": "power outlet",
    "class_name": "power outlet",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 244,
    "name": "remote control",
    "class_name": "remote control",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 253,
    "name": "saxophone",
    "class_name": "saxophone",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 257,
    "name": "screwdriver",
    "class_name": "screwdriver",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 262,
    "name": "shoe",
    "class_name": "shoe",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 264,
    "name": "shovel",
    "class_name": "shovel",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 276,
    "name": "soccer ball",
    "class_name": "soccer ball",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 277,
    "name": "sock",
    "class_name": "sock",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 289,
    "name": "stethoscope",
    "class_name": "stethoscope",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 291,
    "name": "stop sign",
    "class_name": "stop sign",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 293,
    "name": "strawberry",
    "class_name": "strawberry",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 294,
    "name": "streetlight",
    "class_name": "streetlight",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 297,
    "name": "suitcase",
    "class_name": "suitcase",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 303,
    "name": "syringe",
    "class_name": "syringe",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 305,
    "name": "table",
    "class_name": "table",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 306,
    "name": "teapot",
    "class_name": "teapot",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 307,
    "name": "teddy-bear",
    "class_name": "teddy-bear",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 310,
    "name": "tennis racquet",
    "class_name": "tennis racquet",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 311,
    "name": "tent",
    "class_name": "tent",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 321,
    "name": "traffic light",
    "class_name": "traffic light",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 327,
    "name": "trumpet",
    "class_name": "trumpet",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 331,
    "name": "vase",
    "class_name": "vase",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 334,
    "name": "watermelon",
    "class_name": "watermelon",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 336,
    "name": "whale",
    "class_name": "whale",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 339,
    "name": "wine bottle",
    "class_name": "wine bottle",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 340,
    "name": "wine glass",
    "class_name": "wine glass",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 342,
    "name": "stick man",
    "class_name": "stick man",
    "difficulty": "medium",
    "label": "medium"
  },
  {
    "id": 4,
    "name": "airplane",
    "class_name": "airplane",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 9,
    "name": "ant",
    "class_name": "ant",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 29,
    "name": "bee",
    "class_name": "bee",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 36,
    "name": "blackberry",
    "class_name": "blackberry",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 39,
    "name": "boomerang",
    "class_name": "boomerang",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 45,
    "name": "bridge",
    "class_name": "bridge",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 47,
    "name": "broom",
    "class_name": "broom",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 67,
    "name": "cat",
    "class_name": "cat",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 85,
    "name": "crab",
    "class_name": "crab",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 106,
    "name": "elephant",
    "class_name": "elephant",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 120,
    "name": "flamingo",
    "class_name": "flamingo",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 128,
    "name": "frog",
    "class_name": "frog",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 130,
    "name": "garden",
    "class_name": "garden",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 150,
    "name": "horse",
    "class_name": "horse",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 177,
    "name": "lion",
    "class_name": "lion",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 182,
    "name": "map",
    "class_name": "map",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 204,
    "name": "octopus",
    "class_name": "octopus",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 207,
    "name": "owl",
    "class_name": "owl",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 221,
    "name": "penguin",
    "class_name": "penguin",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 223,
    "name": "pickup truck",
    "class_name": "pickup truck",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 237,
    "name": "purse",
    "class_name": "purse",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 238,
    "name": "rabbit",
    "class_name": "rabbit",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 258,
    "name": "sea turtle",
    "class_name": "sea turtle",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 261,
    "name": "sheep",
    "class_name": "sheep",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 279,
    "name": "spider",
    "class_name": "spider",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 296,
    "name": "submarine",
    "class_name": "submarine",
    "difficulty": "hard",
    "label": "hard"
  },
  {
    "id": 299,
    "name": "swan",
    "class_name": "swan",
    "difficulty": "hard",
    "label": "hard"
  }
];

export const CURATED_BLUEPRINTS = {
  "alarm clock": {
    "difficulty": "easy",
    "name": "Alarm Clock",
    "paths": [
      "M 200 110 A 80 80 0 1 0 200 270 A 80 80 0 1 0 200 110 Z",
      "M 200 190 L 200 140",
      "M 200 190 L 235 190",
      "M 140 120 C 120 100 130 80 155 90",
      "M 260 120 C 280 100 270 80 245 90",
      "M 140 260 L 120 290",
      "M 260 260 L 280 290"
    ],
    "tips": "1. Draw circular clock body. 2. Add twin bells on top and diagonal feet. 3. Draw hour and minute hands."
  },
  "anvil": {
    "difficulty": "easy",
    "name": "Anvil",
    "paths": [
      "M 80 160 C 110 155 130 145 150 140 L 300 140 L 300 170 L 240 180 C 230 220 230 250 270 270 L 270 300 L 130 300 L 130 270 C 170 250 170 220 160 180 C 130 180 100 170 80 160 Z"
    ],
    "tips": "Trace in one continuous stroke: start at the horn, draw across the flat top, down to the flared base, and back up."
  },
  "apple": {
    "difficulty": "easy",
    "name": "Apple",
    "paths": [
      "M 200 140 C 160 90 90 130 100 210 C 110 290 170 330 200 330 C 230 330 290 290 300 210 C 310 130 240 90 200 140 Z",
      "M 200 140 Q 210 90 230 70",
      "M 215 100 Q 260 85 245 120 Q 215 110 215 100"
    ],
    "tips": "1. Draw rounded heart-like fruit contour. 2. Add top stem and curved leaf."
  },
  "axe": {
    "difficulty": "easy",
    "name": "Axe",
    "paths": [
      "M 180 60 L 200 60 L 200 340 L 180 340 Z",
      "M 200 80 L 280 60 L 290 140 L 200 120 Z"
    ],
    "tips": "1. Draw straight wooden handle. 2. Add curved cutting blade on top."
  },
  "basket": {
    "difficulty": "easy",
    "name": "Basket",
    "paths": [
      "M 115.3 159.2 L 155.5 193.1 L 181.8 201.9 L 227.0 203.1 L 280.9 179.3 L 314.8 142.9 L 323.6 210.7 L 322.4 265.9 L 308.5 306.0 L 289.7 329.9 L 269.6 344.9 L 255.8 351.2 L 225.7 357.5 L 194.4 357.5 L 159.2 350.0 L 120.3 327.4 L 102.7 311.1 L 85.2 284.7 L 76.4 250.8 L 80.2 229.5 L 91.5 198.1 L 114.0 165.5 L 107.8 166.7",
      "M 132.9 180.5 L 134.1 116.5 L 141.6 92.7 L 155.5 72.6 L 180.5 51.3 L 220.7 40.0 L 260.9 46.3 L 275.9 53.8 L 291.0 68.9 L 307.3 117.8 L 306.0 151.7 L 302.3 155.5",
      "M 139.1 189.3 L 131.6 243.3 L 114.0 321.1 L 107.8 323.6 L 110.3 312.3",
      "M 188.1 200.6 L 183.1 238.3 L 156.7 331.1 L 159.2 338.7 L 166.7 338.7",
      "M 245.8 203.1 L 253.3 232.0 L 239.5 329.9 L 240.8 358.7 L 235.8 360.0",
      "M 291.0 184.3 L 284.7 253.3 L 289.7 337.4"
    ],
    "tips": "Trace outline in 6 clean strokes for instant 90%+ AI recognition."
  },
  "book": {
    "difficulty": "easy",
    "name": "Book",
    "paths": [
      "M 216.9 142.9 L 220.7 210.7 L 205.6 297.3 L 201.9 287.2",
      "M 216.9 141.6 L 191.8 117.8 L 165.5 104.0 L 114.0 88.9 L 102.7 88.9 L 88.9 97.7 L 82.7 112.8 L 81.4 166.7 L 73.9 209.4 L 40.0 298.5 L 56.3 292.2 L 107.8 293.5 L 146.7 302.3 L 208.2 298.5",
      "M 204.4 293.5 L 222.0 253.3 L 228.2 228.2 L 228.2 149.2 L 214.4 142.9 L 219.5 144.2 L 227.0 158.0 L 228.2 183.1 L 213.2 252.1 L 211.9 292.2 L 211.9 245.8 L 230.7 170.5 L 223.2 141.6 L 316.1 86.4 L 347.5 76.4 L 358.7 86.4 L 360.0 140.4 L 352.5 185.6 L 344.9 308.5 L 337.4 323.6 L 313.6 323.6 L 224.5 301.0 L 208.2 293.5 L 206.9 280.9"
    ],
    "tips": "Trace outline in 3 clean strokes for instant 90%+ AI recognition."
  },
  "bowtie": {
    "difficulty": "easy",
    "name": "Bowtie",
    "paths": [
      "M 190 190 L 210 190 L 210 210 L 190 210 Z",
      "M 190 190 L 100 140 L 100 260 L 190 210 Z",
      "M 210 190 L 300 140 L 300 260 L 210 210 Z"
    ],
    "tips": "1. Draw small center knot. 2. Draw two triangular flared wings."
  },
  "bucket": {
    "difficulty": "medium",
    "name": "Bucket",
    "paths": [
      "M 110 160 L 290 160 L 260 330 L 140 330 Z",
      "M 110 160 C 110 60 290 60 290 160"
    ],
    "tips": "1. Draw tapered bucket pail. 2. Add curved overhead handle."
  },
  "cactus": {
    "difficulty": "easy",
    "name": "Cactus",
    "paths": [
      "M 175 340 L 175 250 L 125 250 C 105 250 105 160 125 160 C 145 160 145 220 160 220 L 175 220 L 175 80 C 175 50 225 50 225 80 L 225 180 L 240 180 C 255 180 255 130 275 130 C 295 130 295 210 275 210 L 225 210 L 225 340 Z"
    ],
    "tips": "Trace the entire saguaro cactus silhouette in one continuous stroke: left arm, thicker center stem, right arm."
  },
  "cake": {
    "difficulty": "medium",
    "name": "Cake",
    "paths": [
      "M 100 240 L 300 240 L 300 310 L 100 310 Z",
      "M 130 170 L 270 170 L 270 240 L 130 240 Z",
      "M 200 170 L 200 130",
      "M 200 115 A 8 8 0 1 0 200 116"
    ],
    "tips": "1. Draw stacked cake tiers. 2. Top with a birthday candle."
  },
  "calculator": {
    "difficulty": "easy",
    "name": "Calculator",
    "paths": [
      "M 120 80 L 280 80 L 280 320 L 120 320 Z",
      "M 140 105 L 260 105 L 260 145 L 140 145 Z",
      "M 145 175 L 175 175",
      "M 195 175 L 225 175",
      "M 245 175 L 255 175",
      "M 145 220 L 175 220",
      "M 195 220 L 225 220",
      "M 245 220 L 255 220"
    ],
    "tips": "1. Draw rectangular casing. 2. Add top display screen and button grid."
  },
  "camera": {
    "difficulty": "easy",
    "name": "Camera",
    "paths": [
      "M 80 150 L 320 150 L 320 310 L 80 310 Z",
      "M 200 180 A 50 50 0 1 0 200 280 A 50 50 0 1 0 200 180 Z",
      "M 120 150 L 120 125 L 170 125 L 170 150",
      "M 260 150 A 10 10 0 1 0 260 151"
    ],
    "tips": "1. Draw camera box. 2. Add round center lens and top shutter button."
  },
  "campfire": {
    "difficulty": "easy",
    "name": "Campfire",
    "paths": [
      "M 90 320 L 310 270",
      "M 90 270 L 310 320",
      "M 200 90 C 120 180 130 280 200 280 C 270 280 280 180 200 90 Z"
    ],
    "tips": "1. Draw crossed wood logs at bottom. 2. Add egg-shaped rising flame above."
  },
  "car": {
    "difficulty": "easy",
    "name": "Car",
    "paths": [
      "M 80 250 L 320 250 L 320 210 L 260 190 L 230 130 L 150 130 L 120 190 L 80 210 Z",
      "M 130 250 A 25 25 0 1 0 130 300 A 25 25 0 1 0 130 250 Z",
      "M 270 250 A 25 25 0 1 0 270 300 A 25 25 0 1 0 270 250 Z"
    ],
    "tips": "1. Draw lower car body chassis. 2. Add roof cabin and windows. 3. Add two circular wheels."
  },
  "castle": {
    "difficulty": "easy",
    "name": "Castle",
    "paths": [
      "M 90 140 L 130 140 L 130 320 L 90 320 Z",
      "M 270 140 L 310 140 L 310 320 L 270 320 Z",
      "M 130 200 L 270 200 L 270 320 L 130 320 Z",
      "M 175 260 C 175 240 225 240 225 260 L 225 320 L 175 320 Z"
    ],
    "tips": "1. Draw twin side towers with battlements. 2. Connect with center wall and arched gate."
  },
  "cell phone": {
    "difficulty": "easy",
    "name": "Cell Phone",
    "paths": [
      "M 130 70 L 270 70 L 270 330 L 130 330 Z",
      "M 145 95 L 255 95 L 255 285 L 145 285 Z"
    ],
    "tips": "1. Draw rounded rectangle casing. 2. Add inner screen frame and home button."
  },
  "chair": {
    "difficulty": "easy",
    "name": "Chair",
    "paths": [
      "M 140 80 L 140 230 L 260 230",
      "M 140 230 L 140 340",
      "M 260 230 L 260 340",
      "M 140 130 L 260 130"
    ],
    "tips": "1. Draw vertical backrest. 2. Add horizontal seat and four legs."
  },
  "circle": {
    "difficulty": "easy",
    "name": "Circle",
    "paths": [
      "M 80 200 A 120 120 0 1 0 320 200 A 120 120 0 1 0 80 200 Z"
    ],
    "tips": "Trace a smooth, closed circular contour in one continuous stroke."
  },
  "cloud": {
    "difficulty": "easy",
    "name": "Cloud",
    "paths": [
      "M 80 220 C 50 200 60 160 90 150 C 90 100 150 90 180 120 C 220 80 280 90 290 130 C 330 120 350 160 340 200 C 350 230 320 250 290 240 C 260 260 210 260 190 240 C 160 260 110 260 80 220 Z"
    ],
    "tips": "Trace puffy cloud outline with billowing lobes in one continuous stroke."
  },
  "crown": {
    "difficulty": "easy",
    "name": "Crown",
    "paths": [
      "M 80 280 L 320 280 L 330 160 L 260 210 L 200 130 L 140 210 L 70 160 Z",
      "M 80 300 L 320 300"
    ],
    "tips": "1. Draw horizontal base band. 2. Add 3 to 5 pointed peaks topped with jewels."
  },
  "cup": {
    "difficulty": "easy",
    "name": "Cup",
    "paths": [
      "M 90.8 73.9 L 124.7 86.4 L 176.2 90.2 L 266.5 67.6 L 310.4 51.3 L 302.9 47.5 L 254.0 41.3 L 174.9 40.0 L 126.0 47.5 L 89.6 63.8 L 103.4 104.0 L 109.6 215.7 L 107.1 327.4 L 113.4 338.7 L 133.5 350.0 L 159.8 351.2 L 197.5 360.0 L 292.9 350.0 L 299.1 343.7 L 301.6 164.2 L 309.2 106.5 L 310.4 43.8",
      "M 298 120 C 355 120 355 260 292 260"
    ],
    "tips": "1. Trace cup body outline. 2. Add curved handle on the side."
  },
  "diamond": {
    "difficulty": "easy",
    "name": "Diamond",
    "paths": [
      "M 86.8 76.7 L 53.9 119.7 L 40.0 145.0 L 40.0 150.0 L 57.7 175.3 L 107.0 213.3 L 233.5 330.9 L 237.3 329.6 L 282.8 271.5 L 329.6 186.7 L 360.0 145.0 L 357.5 133.6 L 313.2 70.4 L 242.4 69.1 L 158.9 76.7 L 89.3 74.2"
    ],
    "tips": "Trace outline in one continuous stroke for instant 90%+ AI recognition."
  },
  "door": {
    "difficulty": "easy",
    "name": "Door",
    "paths": [
      "M 120 70 L 280 70 L 280 340 L 120 340 Z",
      "M 250 210 A 8 8 0 1 0 250 215"
    ],
    "tips": "1. Draw tall rectangular door frame. 2. Add circular doorknob."
  },
  "drums": {
    "difficulty": "easy",
    "name": "Drums",
    "paths": [
      "M 120 160 C 120 130 280 130 280 160 C 280 190 120 190 120 160 Z",
      "M 120 160 L 120 280 C 120 310 280 310 280 280 L 280 160",
      "M 120 180 L 200 280 L 280 180",
      "M 90 90 L 160 150",
      "M 310 90 L 240 150"
    ],
    "tips": "1. Draw top circular drum rim. 2. Add cylindrical drum body with diagonal tension ropes."
  },
  "envelope": {
    "difficulty": "easy",
    "name": "Envelope",
    "paths": [
      "M 80 120 L 320 120 L 320 280 L 80 280 Z",
      "M 80 120 L 200 210 L 320 120"
    ],
    "tips": "1. Draw rectangular envelope body. 2. Add top diagonal folded triangular flap."
  },
  "eraser": {
    "difficulty": "easy",
    "name": "Eraser",
    "paths": [
      "M 80 240 L 130 160 L 320 160 L 270 240 Z",
      "M 180 160 L 155 240"
    ],
    "tips": "1. Draw the angled rectangular eraser body. 2. Add the center divider band."
  },
  "eye": {
    "difficulty": "easy",
    "name": "Eye",
    "paths": [
      "M 70 200 Q 200 110 330 200 Q 200 290 70 200 Z",
      "M 200 160 A 40 40 0 1 0 200 240 A 40 40 0 1 0 200 160 Z",
      "M 200 185 A 15 15 0 1 0 200 215 A 15 15 0 1 0 200 185 Z"
    ],
    "tips": "1. Draw almond-shaped eyelids. 2. Add round iris and center pupil."
  },
  "fire hydrant": {
    "difficulty": "easy",
    "name": "Fire Hydrant",
    "paths": [
      "M 150 140 C 150 95 250 95 250 140 L 265 140 L 265 170 L 250 170 L 250 320 L 150 320 L 150 170 L 135 170 L 135 140 Z",
      "M 120 200 L 150 200 L 150 230 L 120 230 Z",
      "M 250 200 L 280 200 L 280 230 L 250 230 Z"
    ],
    "tips": "1. Draw dome-capped central cylinder. 2. Add side nozzle caps and wide base flange."
  },
  "fireplace": {
    "difficulty": "easy",
    "name": "Fireplace",
    "paths": [
      "M 52.6 287.6 L 61.4 195.6 L 81.6 101.1 L 87.9 98.6 L 181.1 98.6 L 240.3 107.4 L 303.3 107.4 L 360.0 97.3 L 356.2 206.9 L 344.9 285.0 L 339.8 292.6 L 331.0 297.6 L 303.3 302.7 L 189.9 302.7 L 71.5 288.8 L 40.0 290.1",
      "M 139.5 280.0 L 129.4 256.1 L 126.9 225.8 L 132.0 214.5 L 137.0 230.9 L 140.8 227.1 L 152.1 167.9 L 154.6 189.3 L 158.4 189.3 L 177.3 157.8 L 197.5 135.1 L 187.4 213.2 L 208.8 175.4 L 236.5 142.7 L 239.1 178.0 L 231.5 206.9 L 236.5 191.8 L 252.9 169.1 L 270.6 154.0 L 273.1 159.1 L 271.8 196.9 L 259.2 224.6 L 271.8 206.9 L 284.4 198.1 L 285.7 222.0 L 274.3 242.2 L 237.8 281.3 L 202.5 297.6 L 178.6 298.9 L 154.6 295.1 L 137.0 287.6 L 130.7 277.5"
    ],
    "tips": "Trace outline in 2 clean strokes for instant 90%+ AI recognition."
  },
  "flashlight": {
    "difficulty": "easy",
    "name": "Flashlight",
    "paths": [
      "M 100 185 L 200 185 L 240 150 L 280 150 L 280 250 L 240 250 L 200 215 L 100 215 Z",
      "M 300 160 L 340 140",
      "M 300 200 L 350 200",
      "M 300 240 L 340 260"
    ],
    "tips": "1. Draw the flashlight handle and flared head. 2. Add the three shining light rays."
  },
  "flip flops": {
    "difficulty": "easy",
    "name": "Flip Flops",
    "paths": [
      "M 110 180 C 110 120 160 120 160 180 C 160 260 120 320 110 320 C 100 320 90 250 110 180 Z",
      "M 135 170 L 110 230",
      "M 135 170 L 160 230",
      "M 240 180 C 240 120 290 120 290 180 C 290 260 250 320 240 320 C 230 320 220 250 240 180 Z",
      "M 265 170 L 240 230",
      "M 265 170 L 290 230"
    ],
    "tips": "1. Draw twin sandal soles. 2. Add V-shaped toe straps on each."
  },
  "floor lamp": {
    "difficulty": "easy",
    "name": "Floor Lamp",
    "paths": [
      "M 130 160 L 270 160 L 240 90 L 160 90 Z",
      "M 200 160 L 200 330",
      "M 140 330 L 260 330"
    ],
    "tips": "1. Draw trapezoid lampshade. 2. Add tall center pole and circular floor base."
  },
  "fork": {
    "difficulty": "easy",
    "name": "Fork",
    "paths": [
      "M 170 80 L 170 170",
      "M 190 80 L 190 170",
      "M 210 80 L 210 170",
      "M 230 80 L 230 170",
      "M 170 170 C 170 200 230 200 230 170",
      "M 200 195 L 200 340"
    ],
    "tips": "1. Draw 4 vertical tines connecting to head. 2. Extend long straight handle down."
  },
  "frying pan": {
    "difficulty": "easy",
    "name": "Frying Pan",
    "paths": [
      "M 80 200 A 70 70 0 1 0 220 200 A 70 70 0 1 0 80 200 Z",
      "M 220 200 L 350 200"
    ],
    "tips": "1. Draw the round pan basin. 2. Add the straight horizontal handle."
  },
  "grass": {
    "difficulty": "easy",
    "name": "Grass",
    "paths": [
      "M 60 280 L 80 180 L 100 280 L 120 160 L 140 280 L 160 170 L 180 280 L 200 150 L 220 280 L 240 170 L 260 280 L 280 160 L 300 280 L 320 180 L 340 280 Z"
    ],
    "tips": "Trace the zigzag blades of grass in one continuous stroke across the canvas."
  },
  "harp": {
    "difficulty": "easy",
    "name": "Harp",
    "paths": [
      "M 130 90 L 130 330 L 270 330 C 280 230 240 150 130 90 Z",
      "M 155 120 L 155 330",
      "M 180 145 L 180 330",
      "M 205 180 L 205 330",
      "M 230 225 L 230 330"
    ],
    "tips": "1. Draw triangular pillar, neck, and soundbox. 2. Add vertical parallel strings."
  },
  "hat": {
    "difficulty": "easy",
    "name": "Hat",
    "paths": [
      "M 80 270 L 320 270",
      "M 130 270 L 140 140 C 140 110 260 110 260 140 L 270 270 Z"
    ],
    "tips": "1. Draw wide oval brim. 2. Add rounded dome crown."
  },
  "headphones": {
    "difficulty": "easy",
    "name": "Headphones",
    "paths": [
      "M 120 220 C 120 110 280 110 280 220",
      "M 100 200 L 130 200 L 130 270 L 100 270 Z",
      "M 270 200 L 300 200 L 300 270 L 270 270 Z"
    ],
    "tips": "1. Draw arched headband. 2. Add padded earcups on both sides."
  },
  "hockey stick": {
    "difficulty": "easy",
    "name": "Hockey Stick",
    "paths": [
      "M 260 60 L 140 280 L 80 280 C 70 280 70 300 80 300 L 160 300 L 280 80 Z"
    ],
    "tips": "Draw the angled shaft and curved hockey blade in one continuous stroke."
  },
  "hourglass": {
    "difficulty": "easy",
    "name": "Hourglass",
    "paths": [
      "M 130 90 L 270 90",
      "M 130 310 L 270 310",
      "M 140 90 L 200 200 L 140 310",
      "M 260 90 L 200 200 L 260 310"
    ],
    "tips": "1. Draw top and bottom horizontal plates. 2. Add intersecting diagonal glass bulbs."
  },
  "house": {
    "difficulty": "easy",
    "name": "House",
    "paths": [
      "M 90 190 L 200 90 L 310 190 Z",
      "M 110 190 L 110 330 L 290 330 L 290 190 Z",
      "M 175 240 L 225 240 L 225 330 L 175 330 Z"
    ],
    "tips": "1. Draw triangular roof. 2. Add square walls and center door."
  },
  "house plant": {
    "difficulty": "easy",
    "name": "House Plant",
    "paths": [
      "M 176.2 254.6 L 216.3 258.4 L 274.0 248.3 L 276.5 272.2 L 256.5 331.1 L 241.4 358.7 L 190.0 360.0 L 188.7 322.4 L 177.4 248.3",
      "M 195.0 252.1 L 187.5 195.6 L 151.1 140.4 L 123.5 114.0 L 129.7 112.8 L 143.5 120.3 L 181.2 152.9 L 183.7 158.0 L 178.7 184.3 L 187.5 116.5 L 198.7 97.7 L 232.6 60.1 L 221.3 106.5 L 186.2 150.4 L 178.7 134.1 L 164.9 117.8 L 161.1 100.2 L 162.4 65.1 L 173.6 40.0 L 184.9 75.1 L 186.2 104.0 L 192.5 120.3"
    ],
    "tips": "Trace outline in 2 clean strokes for instant 90%+ AI recognition."
  },
  "hurricane": {
    "difficulty": "easy",
    "name": "Hurricane",
    "paths": [
      "M 94.2 55.1 L 152.1 41.3 L 186.1 40.0 L 294.5 45.0 L 318.4 53.9 L 336.1 75.3 L 339.8 119.4 L 278.1 167.2 L 167.2 168.5 L 129.4 162.2 L 62.7 133.2 L 56.4 121.9 L 61.4 104.3 L 116.9 72.8 L 245.4 56.4 L 292.0 62.7 L 336.1 82.8 L 343.6 96.7 L 339.8 106.8 L 255.4 166.0 L 145.8 181.1 L 104.3 157.2 L 98.0 123.1 L 147.1 80.3 L 171.0 75.3 L 210.1 77.8 L 246.6 87.9 L 295.7 119.4 L 237.8 171.0 L 158.4 189.9 L 125.7 188.7 L 162.2 164.7 L 197.5 153.4 L 227.7 149.6 L 250.4 152.1 L 260.5 158.4 L 264.3 177.3 L 237.8 200.0 L 213.9 210.1 L 169.8 212.6 L 144.6 205.0 L 178.6 184.9 L 210.1 174.8 L 273.1 177.3 L 286.9 183.6 L 292.0 193.7 L 258.0 225.2 L 198.7 242.8 L 177.3 245.4 L 162.2 244.1 L 164.7 232.8 L 234.0 203.8 L 258.0 205.0 L 275.6 220.2 L 273.1 227.7 L 210.1 270.6 L 184.9 276.9 L 157.2 270.6 L 169.8 251.7 L 197.5 235.3 L 265.5 226.5 L 265.5 254.2 L 222.7 285.7 L 181.1 297.0 L 169.8 286.9 L 213.9 269.3 L 237.8 265.5 L 263.0 269.3 L 236.5 304.6 L 187.4 328.5 L 177.3 328.5 L 172.3 324.7 L 195.0 302.0 L 213.9 294.5 L 249.1 293.2 L 256.7 298.3 L 249.1 331.0 L 216.4 355.0 L 189.9 355.0 L 231.5 324.7 L 260.5 331.0 L 256.7 341.1 L 218.9 360.0 L 205.0 358.7"
    ],
    "tips": "Trace outline in one continuous stroke for instant 90%+ AI recognition."
  },
  "ice cream": {
    "difficulty": "easy",
    "name": "Ice Cream",
    "paths": [
      "M 150 180 L 250 180 L 200 340 Z",
      "M 200 180 A 50 50 0 1 0 200 80 A 50 50 0 1 0 200 180 Z"
    ],
    "tips": "1. Draw triangular cone. 2. Add round ice cream scoop on top."
  },
  "jacket": {
    "difficulty": "easy",
    "name": "Jacket",
    "paths": [
      "M 150 100 Q 200 130 250 100 L 320 150 L 290 190 L 260 170 L 260 330 L 140 330 L 140 170 L 110 190 L 80 150 Z",
      "M 200 130 L 200 330"
    ],
    "tips": "1. Draw collar and front zipper line. 2. Add left and right sleeves and waist."
  },
  "jail": {
    "difficulty": "easy",
    "name": "Jail",
    "paths": [
      "M 100 100 L 300 100 L 300 300 L 100 300 Z",
      "M 140 100 L 140 300",
      "M 180 100 L 180 300",
      "M 220 100 L 220 300",
      "M 260 100 L 260 300"
    ],
    "tips": "1. Draw outer rectangular window. 2. Add vertical iron bars."
  },
  "ladder": {
    "difficulty": "easy",
    "name": "Ladder",
    "paths": [
      "M 140 70 L 140 340",
      "M 260 70 L 260 340",
      "M 140 120 L 260 120",
      "M 140 180 L 260 180",
      "M 140 240 L 260 240",
      "M 140 300 L 260 300"
    ],
    "tips": "1. Draw two vertical side rails. 2. Add horizontal rungs between them."
  },
  "light bulb": {
    "difficulty": "easy",
    "name": "Light Bulb",
    "paths": [
      "M 150 230 C 110 200 90 130 140 80 C 190 40 260 60 280 110 C 295 155 265 205 230 230 L 230 260 L 150 260 Z",
      "M 155 260 L 225 260 L 210 330 L 170 330 Z",
      "M 163 285 L 217 285",
      "M 168 310 L 212 310"
    ],
    "tips": "1. Draw round bulb body tapering to neck. 2. Draw a long trapezium screw base. 3. Add 2 horizontal lines inside the trapezium."
  },
  "lightning": {
    "difficulty": "easy",
    "name": "Lightning",
    "paths": [
      "M 220 60 L 140 200 L 200 200 L 170 340 L 260 180 L 200 180 Z"
    ],
    "tips": "1. Draw zigzag electric bolt striking downwards."
  },
  "microwave": {
    "difficulty": "easy",
    "name": "Microwave",
    "paths": [
      "M 90 130 L 310 130 L 310 290 L 90 290 Z",
      "M 110 150 L 240 150 L 240 270 L 110 270 Z",
      "M 265 170 L 285 170",
      "M 265 200 L 285 200"
    ],
    "tips": "1. Draw rectangular outer body. 2. Add inner glass window door and keypad buttons."
  },
  "necklace": {
    "difficulty": "medium",
    "name": "Necklace",
    "paths": [
      "M 100 100 C 100 270 300 270 300 100",
      "M 200 240 A 18 18 0 1 0 200 276 A 18 18 0 1 0 200 240 Z"
    ],
    "tips": "1. Draw curved necklace chain. 2. Add gemstone pendant hanging at center."
  },
  "onion": {
    "difficulty": "easy",
    "name": "Onion",
    "paths": [
      "M 188.7 40.0 L 188.7 65.1 L 178.7 75.1 L 157.3 83.9 L 119.7 92.7 L 92.1 117.8 L 75.8 155.5 L 58.2 220.7 L 56.9 252.1 L 75.8 291.0 L 93.3 311.1 L 118.4 326.1 L 169.9 338.7 L 221.3 337.4 L 286.6 307.3 L 330.5 269.6 L 343.1 233.3 L 341.8 199.4 L 330.5 183.1 L 300.4 158.0 L 223.8 104.0 L 191.2 48.8",
      "M 186.2 68.9 L 144.8 107.8 L 110.9 181.8 L 90.8 280.9 L 89.6 339.9 L 95.8 360.0",
      "M 184.9 73.9 L 182.4 114.0 L 183.7 163.0 L 206.3 222.0 L 231.4 274.7"
    ],
    "tips": "Trace outline in 3 clean strokes for instant 90%+ AI recognition."
  },
  "palm tree": {
    "difficulty": "easy",
    "name": "Palm Tree",
    "paths": [
      "M 190 170 Q 180 260 160 340 L 210 340 Q 210 260 210 170 Z",
      "M 200 170 Q 130 100 60 140",
      "M 200 170 Q 130 160 70 210",
      "M 200 170 Q 270 100 340 140",
      "M 200 170 Q 270 160 330 210"
    ],
    "tips": "1. Draw slender curved trunk. 2. Add long arching palm frond leaves."
  },
  "pants": {
    "difficulty": "easy",
    "name": "Pants",
    "paths": [
      "M 130 90 L 270 90 L 285 330 L 225 330 L 200 180 L 175 330 L 115 330 Z"
    ],
    "tips": "1. Draw waistband. 2. Extend two vertical leg pipes with center inseam."
  },
  "pencil": {
    "difficulty": "easy",
    "name": "Pencil",
    "paths": [
      "M 190.6 86.6 L 167.9 324.7 L 194.3 360.0 L 213.2 338.6 L 222.0 294.5 L 224.6 86.6 L 193.1 89.1",
      "M 186.8 95.4 L 200.6 41.3 L 232.1 40.0 L 232.1 51.3 L 222.0 82.8",
      "M 178.0 319.7 L 201.9 312.1 L 224.6 312.1"
    ],
    "tips": "Trace outline in 3 clean strokes for instant 90%+ AI recognition."
  },
  "picture frame": {
    "difficulty": "easy",
    "name": "Picture Frame",
    "paths": [
      "M 90 90 L 310 90 L 310 310 L 90 310 Z",
      "M 120 120 L 280 120 L 280 280 L 120 280 Z"
    ],
    "tips": "1. Draw outer rectangular frame. 2. Add inner photo border."
  },
  "radio": {
    "difficulty": "easy",
    "name": "Radio",
    "paths": [
      "M 90 160 L 310 160 L 310 300 L 90 300 Z",
      "M 120 160 L 70 80",
      "M 160 230 A 35 35 0 1 0 160 231",
      "M 240 190 L 280 190",
      "M 240 220 L 280 220",
      "M 240 250 L 280 250"
    ],
    "tips": "1. Draw the radio body. 2. Add diagonal antenna and speaker circle."
  },
  "rain": {
    "difficulty": "easy",
    "name": "Rain",
    "paths": [
      "M 197.5 40.0 L 197.5 63.8",
      "M 195.0 168.0 L 198.7 180.5 L 200.0 237.0",
      "M 203.8 303.5 L 205.0 360.0"
    ],
    "tips": "Trace outline in 3 clean strokes for instant 90%+ AI recognition."
  },
  "rainbow": {
    "difficulty": "easy",
    "name": "Rainbow",
    "paths": [
      "M 80 300 C 80 140 320 140 320 300",
      "M 110 300 C 110 170 290 170 290 300",
      "M 140 300 C 140 200 260 200 260 300"
    ],
    "tips": "1. Draw 3 nested concentric arched bands arching across sky."
  },
  "rollerskates": {
    "difficulty": "easy",
    "name": "Rollerskates",
    "paths": [
      "M 100 80 L 180 80 L 180 190 L 290 190 C 310 220 300 240 270 240 L 100 240 Z",
      "M 95 240 L 285 240",
      "M 130 285 A 25 25 0 1 0 130 286",
      "M 250 285 A 25 25 0 1 0 250 286"
    ],
    "tips": "1. Draw large high-top skate boot and sole. 2. Add two wheels below."
  },
  "sailboat": {
    "difficulty": "easy",
    "name": "Sailboat",
    "paths": [
      "M 90 260 L 310 260 L 270 320 L 130 320 Z",
      "M 200 90 L 200 260",
      "M 200 100 L 280 240 L 200 240 Z",
      "M 200 120 L 130 240 L 200 240 Z"
    ],
    "tips": "1. Draw curved boat hull. 2. Add vertical mast and triangular wind sail."
  },
  "scissors": {
    "difficulty": "easy",
    "name": "Scissors",
    "paths": [
      "M 120 130 A 25 25 0 1 0 120 140",
      "M 120 270 A 25 25 0 1 0 120 280",
      "M 140 135 L 290 270",
      "M 140 275 L 290 130"
    ],
    "tips": "1. Draw twin finger loop handles. 2. Add crossing straight blades."
  },
  "see saw": {
    "difficulty": "easy",
    "name": "See Saw",
    "paths": [
      "M 180 320 L 200 250 L 220 320 Z",
      "M 70 220 L 330 280",
      "M 140 320 L 260 320"
    ],
    "tips": "1. Draw center triangle fulcrum. 2. Add tilted horizontal plank."
  },
  "skull": {
    "difficulty": "easy",
    "name": "Skull",
    "paths": [
      "M 120 200 C 80 130 150 70 200 70 C 250 70 320 130 280 200 C 270 230 240 230 240 270 L 160 270 C 160 230 130 230 120 200 Z",
      "M 160 170 A 18 18 0 1 0 160 171",
      "M 240 170 A 18 18 0 1 0 240 171",
      "M 200 200 L 190 220 L 210 220 Z",
      "M 180 270 L 180 255",
      "M 200 270 L 200 255",
      "M 220 270 L 220 255"
    ],
    "tips": "1. Draw the cranium and jaw. 2. Add eye sockets and nose cavity."
  },
  "smiley face": {
    "difficulty": "easy",
    "name": "Smiley Face",
    "paths": [
      "M 200 80 A 120 120 0 1 0 200 320 A 120 120 0 1 0 200 80 Z",
      "M 160 165 A 8 8 0 1 0 160 181 A 8 8 0 1 0 160 165 Z",
      "M 240 165 A 8 8 0 1 0 240 181 A 8 8 0 1 0 240 165 Z",
      "M 150 220 Q 200 275 250 220"
    ],
    "tips": "1. Draw outer circle face. 2. Add twin dot eyes. 3. Add curved upward smile."
  },
  "snail": {
    "difficulty": "hard",
    "name": "Snail",
    "paths": [
      "M 200 180 A 60 60 0 1 0 200 300 A 60 60 0 1 0 200 180 Z",
      "M 200 210 A 30 30 0 1 0 200 270 A 30 30 0 1 0 200 210 Z",
      "M 100 300 L 310 300 C 330 300 340 270 330 240 L 300 240",
      "M 325 240 L 335 210",
      "M 315 240 L 310 210"
    ],
    "tips": "1. Draw circular spiral shell. 2. Add crawling foot and eye stalks."
  },
  "snowflake": {
    "difficulty": "easy",
    "name": "Snowflake",
    "paths": [
      "M 200 80 L 200 320",
      "M 80 200 L 320 200",
      "M 115 115 L 285 285",
      "M 115 285 L 285 115"
    ],
    "tips": "1. Draw intersecting radial arms. 2. Add branch chevrons on arm tips."
  },
  "snowman": {
    "difficulty": "easy",
    "name": "Snowman",
    "paths": [
      "M 200 110 A 40 40 0 1 0 200 190 A 40 40 0 1 0 200 110 Z",
      "M 200 190 A 65 65 0 1 0 200 320 A 65 65 0 1 0 200 190 Z"
    ],
    "tips": "1. Draw head circle and larger body circle. 2. Add dot eyes, carrot nose, and hat."
  },
  "spreadsheet": {
    "difficulty": "easy",
    "name": "Spreadsheet",
    "paths": [
      "M 90 90 L 310 90 L 310 310 L 90 310 Z",
      "M 90 145 L 310 145",
      "M 90 200 L 310 200",
      "M 90 255 L 310 255",
      "M 160 90 L 160 310",
      "M 235 90 L 235 310"
    ],
    "tips": "1. Draw outer table box. 2. Add grid rows and columns."
  },
  "stairs": {
    "difficulty": "easy",
    "name": "Stairs",
    "paths": [
      "M 80 320 L 80 260 L 130 260 L 130 210 L 180 210 L 180 160 L 230 160 L 230 110 L 280 110 L 280 320 Z"
    ],
    "tips": "Trace the ascending zigzag steps in one continuous stroke from bottom to top."
  },
  "star": {
    "difficulty": "easy",
    "name": "Star",
    "paths": [
      "M 200 70 L 235 160 L 330 165 L 255 225 L 285 320 L 200 265 L 115 320 L 145 225 L 70 165 L 165 160 Z"
    ],
    "tips": "1. Draw five pointed star vertices radiating from center."
  },
  "sun": {
    "difficulty": "easy",
    "name": "Sun",
    "paths": [
      "M 200 130 A 70 70 0 1 0 200 270 A 70 70 0 1 0 200 130 Z",
      "M 200 60 L 200 100",
      "M 200 300 L 200 340",
      "M 60 200 L 100 200",
      "M 300 200 L 340 200"
    ],
    "tips": "1. Draw center circle disk. 2. Add radiating spoke rays."
  },
  "swing set": {
    "difficulty": "easy",
    "name": "Swing Set",
    "paths": [
      "M 120 100 L 80 320",
      "M 120 100 L 150 320",
      "M 280 100 L 250 320",
      "M 280 100 L 320 320",
      "M 100 100 L 300 100",
      "M 170 100 L 170 260",
      "M 230 100 L 230 260",
      "M 160 260 L 240 260"
    ],
    "tips": "1. Draw two A-frame poles and top beam. 2. Hang swing ropes and seat."
  },
  "sword": {
    "difficulty": "easy",
    "name": "Sword",
    "paths": [
      "M 200 60 L 215 90 L 215 270 L 185 270 L 185 90 Z",
      "M 150 270 L 250 270 L 250 285 L 150 285 Z",
      "M 192 285 L 192 330 L 208 330 L 208 285 Z"
    ],
    "tips": "1. Draw long straight blade with pointed tip. 2. Add crossguard and grip handle."
  },
  "television": {
    "difficulty": "easy",
    "name": "Television",
    "paths": [
      "M 90 130 L 310 130 L 310 290 L 90 290 Z",
      "M 110 150 L 250 150 L 250 270 L 110 270 Z",
      "M 170 90 L 200 130 L 230 90"
    ],
    "tips": "1. Draw outer rectangular TV casing. 2. Add screen, side dials, and V antenna."
  },
  "the eiffel tower": {
    "difficulty": "easy",
    "name": "The Eiffel Tower",
    "paths": [
      "M 120 330 L 180 160 L 195 70 L 205 70 L 220 160 L 280 330",
      "M 145 330 Q 200 270 255 330",
      "M 155 240 L 245 240",
      "M 175 160 L 225 160",
      "M 200 70 L 200 30",
      "M 155 240 L 220 160",
      "M 245 240 L 180 160"
    ],
    "tips": "1. Draw arched base legs. 2. Taper tower upwards through two platforms to the spire."
  },
  "the mona lisa": {
    "difficulty": "easy",
    "name": "The Mona Lisa",
    "paths": [
      "M 100 70 L 300 70 L 300 330 L 100 330 Z",
      "M 160 145 C 160 120 240 120 240 145 C 240 195 160 195 160 145 Z",
      "M 160 140 C 145 170 145 240 160 260",
      "M 240 140 C 255 170 255 240 240 260",
      "M 185 180 Q 200 190 215 180",
      "M 155 250 L 120 315",
      "M 245 250 L 280 315",
      "M 165 290 Q 200 275 235 290"
    ],
    "tips": "1. Draw outer rectangular portrait frame. 2. Draw head with center-parted hair. 3. Add soft smile, eyes, and folded hands."
  },
  "tooth": {
    "difficulty": "easy",
    "name": "Tooth",
    "paths": [
      "M 150 100 C 140 180 150 280 170 320 C 180 290 190 220 200 220 C 210 220 220 290 230 320 C 250 280 260 180 250 100 C 230 90 210 110 200 110 C 190 110 170 90 150 100 Z"
    ],
    "tips": "1. Draw upper molar crown. 2. Add two root prongs extending down."
  },
  "tree": {
    "difficulty": "easy",
    "name": "Tree",
    "paths": [
      "M 180 220 L 180 340",
      "M 220 220 L 220 340",
      "M 140 220 C 100 200 100 140 140 130 C 130 80 190 70 200 90 C 220 70 270 80 260 130 C 300 140 300 200 260 220 Z"
    ],
    "tips": "1. Draw vertical trunk. 2. Add leafy rounded crown canopy."
  },
  "umbrella": {
    "difficulty": "easy",
    "name": "Umbrella",
    "paths": [
      "M 80 210 C 80 120 320 120 320 210 Z",
      "M 200 210 L 200 310 C 200 330 180 330 180 310",
      "M 200 120 L 200 100"
    ],
    "tips": "1. Draw semicircular dome canopy. 2. Add center pole with curved J-handle."
  },
  "ambulance": {
    "difficulty": "medium",
    "name": "Ambulance",
    "paths": [
      "M 40 250 L 40 130 L 240 130 L 300 180 L 360 180 L 360 250 Z",
      "M 105 250 A 28 28 0 1 0 105 251",
      "M 295 250 A 28 28 0 1 0 295 251",
      "M 145 170 L 170 170 L 170 150 L 190 150 L 190 170 L 215 170 L 215 190 L 190 190 L 190 210 L 170 210 L 170 190 L 145 190 Z",
      "M 160 130 L 160 110 L 185 110 L 185 130"
    ],
    "tips": "1. Draw large van body and wheels. 2. Add medical cross and roof siren."
  },
  "angel": {
    "difficulty": "medium",
    "name": "Angel",
    "paths": [
      "M 170 80 A 30 10 0 1 0 230 80 A 30 10 0 1 0 170 80 Z",
      "M 180 120 A 20 20 0 1 0 220 120 A 20 20 0 1 0 180 120 Z",
      "M 185 140 L 130 310 L 270 310 L 215 140 Z",
      "M 180 160 C 120 120 80 180 140 220",
      "M 220 160 C 280 120 320 180 260 220"
    ],
    "tips": "1. Draw halo oval over head circle. 2. Add triangular gown robe. 3. Add outspread wings."
  },
  "asparagus": {
    "difficulty": "medium",
    "name": "Asparagus",
    "paths": [
      "M 199.4 343.7 L 196.9 239.5 L 190.6 184.3 L 179.3 155.5 L 186.8 149.2 L 188.1 134.1 L 193.1 131.6 L 188.1 111.5 L 195.6 106.5 L 194.4 96.5 L 189.3 81.4 L 195.6 70.1 L 198.1 40.0 L 211.9 41.3 L 211.9 62.6 L 220.7 71.4 L 219.5 120.3 L 206.9 165.5 L 210.7 163.0 L 215.7 178.0 L 218.2 195.6 L 220.7 327.4 L 218.2 339.9 L 208.2 356.2 L 201.9 360.0 L 191.8 348.7"
    ],
    "tips": "Trace outline in one continuous stroke for instant 90%+ AI recognition."
  },
  "backpack": {
    "difficulty": "medium",
    "name": "Backpack",
    "paths": [
      "M 152.1 99.8 L 144.6 102.4 L 133.2 285.0 L 133.2 316.5 L 135.7 324.1 L 143.3 330.4 L 179.8 335.4 L 307.1 336.7 L 327.2 332.9 L 341.1 325.4 L 355.0 301.4 L 360.0 258.6 L 360.0 132.6 L 356.2 112.4 L 346.1 98.6 L 300.8 86.0 L 193.7 74.6 L 132.0 74.6 L 108.0 79.7",
      "M 149.6 293.9 L 157.2 272.4 L 171.0 256.1 L 206.3 234.6 L 232.8 229.6 L 308.3 232.1 L 336.1 239.7 L 341.1 246.0 L 342.4 312.8",
      "M 149.6 77.2 L 135.7 65.8 L 101.7 63.3 L 84.1 73.4 L 57.6 104.9 L 46.3 133.9 L 40.0 188.0 L 40.0 213.2 L 48.8 258.6 L 48.8 293.9 L 85.4 312.8 L 66.5 267.4 L 63.9 199.4 L 75.3 149.0 L 87.9 120.0 L 99.2 104.9 L 108.0 101.1 L 138.3 89.8 L 152.1 89.8"
    ],
    "tips": "Trace outline in 3 clean strokes for instant 90%+ AI recognition."
  },
  "banana": {
    "difficulty": "medium",
    "name": "Banana",
    "paths": [
      "M 110 160 C 150 120 280 160 300 270 C 240 280 160 240 110 160 Z",
      "M 110 160 L 90 150 L 100 170 Z",
      "M 120 170 C 170 180 250 210 290 270"
    ],
    "tips": "1. Draw long crescent fruit curve. 2. Add square stem cap and pointed tip. 3. Draw ridge line."
  },
  "baseball": {
    "difficulty": "medium",
    "name": "Baseball",
    "paths": [
      "M 200 80 A 120 120 0 1 0 200 320 A 120 120 0 1 0 200 80 Z",
      "M 150 95 Q 180 200 150 305",
      "M 250 95 Q 220 200 250 305"
    ],
    "tips": "1. Draw circle. 2. Add two curved inward seams. 3. Add stitch tick marks."
  },
  "basketball": {
    "difficulty": "medium",
    "name": "Basketball",
    "paths": [
      "M 200 80 A 120 120 0 1 0 200 320 A 120 120 0 1 0 200 80 Z",
      "M 200 80 L 200 320",
      "M 80 200 L 320 200",
      "M 140 100 Q 170 200 140 300",
      "M 260 100 Q 230 200 260 300"
    ],
    "tips": "1. Draw circle. 2. Add vertical and horizontal center lines. 3. Add curved side lines."
  },
  "bathtub": {
    "difficulty": "medium",
    "name": "Bathtub",
    "paths": [
      "M 70 180 L 330 180 C 330 250 310 270 270 270 L 130 270 C 90 270 70 250 70 180 Z",
      "M 100 270 L 90 310",
      "M 130 270 L 140 310",
      "M 270 270 L 260 310",
      "M 300 270 L 310 310",
      "M 90 180 L 90 120 L 120 120 L 120 140"
    ],
    "tips": "1. Draw deep tub basin and feet. 2. Add overhead faucet."
  },
  "beard": {
    "difficulty": "medium",
    "name": "Beard",
    "paths": [
      "M 130 150 C 130 290 270 290 270 150 C 240 210 160 210 130 150 Z",
      "M 170 180 Q 200 200 230 180"
    ],
    "tips": "1. Draw facial jaw contour. 2. Add bushy textured chin beard."
  },
  "bed": {
    "difficulty": "medium",
    "name": "Bed",
    "paths": [
      "M 90 150 L 90 290",
      "M 310 200 L 310 290",
      "M 90 240 L 310 240",
      "M 90 210 L 310 210",
      "M 110 210 L 160 210 L 160 190 L 110 190 Z"
    ],
    "tips": "1. Draw tall headboard post and footboard. 2. Add mattress horizontal lines and pillow."
  },
  "bee": {
    "difficulty": "hard",
    "name": "Bee",
    "paths": [
      "M 80 210 C 80 150 160 140 260 170 C 310 185 310 235 260 250 C 160 280 80 270 80 210 Z",
      "M 140 152 C 160 180 160 240 140 268",
      "M 190 150 C 210 180 210 240 190 270",
      "M 240 160 C 255 180 255 235 240 258",
      "M 170 145 C 150 70 210 70 190 145",
      "M 200 150 C 190 80 250 80 225 155"
    ],
    "tips": "1. Draw horizontal egg body with face on left. 2. Add curved head line and stripes. 3. Add oval wings on top."
  },
  "bicycle": {
    "difficulty": "medium",
    "name": "Bicycle",
    "paths": [
      "M 110 260 A 35 35 0 1 0 110 261",
      "M 290 260 A 35 35 0 1 0 290 261",
      "M 110 260 L 170 190 L 240 190 L 290 260",
      "M 170 190 L 210 260 L 110 260",
      "M 210 260 L 240 190",
      "M 160 180 L 180 180",
      "M 235 180 L 245 150 L 260 150"
    ],
    "tips": "1. Draw two wheel circles. 2. Connect with triangular frame bars. 3. Add handlebar and seat."
  },
  "binoculars": {
    "difficulty": "medium",
    "name": "Binoculars",
    "paths": [
      "M 110 130 L 170 130 L 180 270 L 100 270 Z",
      "M 230 130 L 290 130 L 300 270 L 220 270 Z",
      "M 170 180 L 230 180",
      "M 170 220 L 230 220"
    ],
    "tips": "1. Draw two side-by-side barrels. 2. Add center hinge bridge and lenses."
  },
  "bracelet": {
    "difficulty": "medium",
    "name": "Bracelet",
    "paths": [
      "M 89.3 77.3 L 79.2 86.2 L 46.3 129.2 L 40.0 158.3 L 43.8 200.0 L 62.8 239.2 L 90.6 264.5 L 137.4 287.3 L 174.1 297.4 L 267.7 303.7 L 306.9 299.9 L 319.5 288.5 L 323.3 278.4 L 333.4 206.3 L 324.6 168.4 L 319.5 159.5 L 285.4 126.6 L 252.5 108.9 L 129.8 74.8 L 94.4 71.0 L 71.6 84.9",
      "M 166.5 64.7 L 156.4 73.5 L 150.0 93.8 L 155.1 103.9 L 165.2 112.7 L 188.0 120.3 L 191.8 108.9 L 165.2 58.3",
      "M 294.2 112.7 L 281.6 106.4 L 261.3 103.9 L 250.0 112.7 L 248.7 121.6 L 255.0 135.5 L 287.9 145.6 L 298.0 136.8 L 305.6 117.8 L 300.6 110.2",
      "M 339.8 187.4 L 319.5 203.8 L 314.5 226.6 L 320.8 248.1 L 329.6 258.2 L 341.0 267.0 L 360.0 265.8 L 360.0 245.5 L 339.8 200.0 L 333.4 189.9 L 323.3 189.9 L 318.3 193.7",
      "M 282.8 268.3 L 239.8 296.1 L 223.4 320.2 L 223.4 331.5 L 238.6 340.4 L 250.0 341.7 L 262.6 339.1 L 287.9 326.5 L 294.2 305.0 L 294.2 278.4 L 285.4 258.2 L 275.3 258.2",
      "M 113.4 219.0 L 98.2 241.7 L 94.4 280.9 L 98.2 305.0 L 104.5 316.4 L 117.2 324.0 L 164.0 329.0 L 175.3 322.7 L 181.7 299.9 L 176.6 278.4 L 162.7 251.9 L 129.8 231.6 L 85.5 222.8"
    ],
    "tips": "Trace outline in 6 clean strokes for instant 90%+ AI recognition."
  },
  "brain": {
    "difficulty": "medium",
    "name": "Brain",
    "paths": [
      "M 130 220 C 100 160 150 110 200 110 C 250 110 300 160 270 220 C 250 260 230 270 200 270 C 170 270 150 260 130 220 Z",
      "M 200 110 L 200 270",
      "M 160 150 Q 180 180 150 210",
      "M 240 150 Q 220 180 250 210"
    ],
    "tips": "1. Draw cranial dome contour. 2. Fill with squiggly wrinkle lobes."
  },
  "bread": {
    "difficulty": "medium",
    "name": "Bread",
    "paths": [
      "M 100 270 L 300 270 C 330 270 330 190 300 170 C 260 130 140 130 100 170 C 70 190 70 270 100 270 Z",
      "M 150 160 L 170 210",
      "M 200 150 L 220 205",
      "M 250 160 L 270 210"
    ],
    "tips": "1. Draw curved loaf contour. 2. Add diagonal crust score marks."
  },
  "bus": {
    "difficulty": "medium",
    "name": "Bus",
    "paths": [
      "M 70 240 L 70 140 L 330 140 L 330 240 Z",
      "M 110 240 A 25 25 0 1 0 160 240",
      "M 240 240 A 25 25 0 1 0 290 240",
      "M 90 160 L 140 160 L 140 190 L 90 190 Z",
      "M 160 160 L 210 160 L 210 190 L 160 190 Z",
      "M 230 160 L 280 160 L 280 190 L 230 190 Z"
    ],
    "tips": "1. Draw long rectangular coach body. 2. Add row of windows and two wheels."
  },
  "bush": {
    "difficulty": "medium",
    "name": "Bush",
    "paths": [
      "M 120.3 257.7 L 95.2 256.5 L 50.0 235.1 L 43.8 228.9 L 40.0 201.3 L 46.3 191.2 L 58.8 188.7 L 56.3 152.3 L 58.8 143.5 L 82.7 133.5 L 99.0 134.7 L 105.3 128.5 L 116.5 124.7 L 166.7 120.9 L 171.8 126.0 L 166.7 123.5 L 168.0 118.4 L 186.8 105.9 L 247.1 107.1 L 264.6 119.7 L 268.4 138.5 L 288.5 129.7 L 302.3 129.7 L 333.6 148.5 L 347.5 171.1 L 350.0 186.2 L 346.2 193.7 L 328.6 196.2 L 339.9 210.0 L 341.2 217.6 L 318.6 226.4 L 353.7 238.9 L 360.0 246.4 L 358.7 255.2 L 348.7 261.5 L 317.3 265.3 L 331.1 277.8 L 331.1 285.3 L 326.1 287.8 L 296.0 294.1 L 252.1 291.6 L 247.1 287.8"
    ],
    "tips": "Trace outline in one continuous stroke for instant 90%+ AI recognition."
  },
  "butterfly": {
    "difficulty": "medium",
    "name": "Butterfly",
    "paths": [
      "M 200 120 L 200 280",
      "M 200 140 C 280 60 340 140 280 200 C 330 240 290 280 200 240 C 110 280 70 240 120 200 C 60 140 120 60 200 140 Z",
      "M 200 120 L 180 90",
      "M 200 120 L 220 90"
    ],
    "tips": "1. Draw center body. 2. Add four symmetrical butterfly wings."
  },
  "candle": {
    "difficulty": "medium",
    "name": "Candle",
    "paths": [
      "M 170 160 L 230 160 L 230 330 L 170 330 Z",
      "M 200 160 L 200 140",
      "M 200 140 Q 185 110 200 80 Q 215 110 200 140 Z"
    ],
    "tips": "1. Draw tall wax cylinder. 2. Add wick and teardrop flame on top."
  },
  "carrot": {
    "difficulty": "medium",
    "name": "Carrot",
    "paths": [
      "M 170 130 L 230 130 L 205 340 L 195 340 Z",
      "M 200 130 L 170 70",
      "M 200 130 L 200 60",
      "M 200 130 L 230 70"
    ],
    "tips": "1. Draw tapered cone root. 2. Add feathery green leaves at top."
  },
  "church": {
    "difficulty": "medium",
    "name": "Church",
    "paths": [
      "M 170 170 L 170 330 L 310 330 L 310 230 L 240 170 Z",
      "M 110 330 L 110 150 L 140 80 L 170 150 L 170 330 Z",
      "M 140 80 L 140 50",
      "M 130 60 L 150 60",
      "M 130 250 L 150 250 L 150 330 L 130 330 Z"
    ],
    "tips": "1. Draw chapel building. 2. Add tall steeple tower with cross on top."
  },
  "compass": {
    "difficulty": "medium",
    "name": "Compass",
    "paths": [
      "M 145.4 339.9 L 115.3 331.1 L 82.7 312.3 L 61.3 288.5 L 43.8 255.8 L 40.0 233.3 L 41.3 209.4 L 63.8 154.2 L 65.1 139.1 L 81.4 120.3 L 129.1 92.7 L 196.9 63.8 L 234.5 60.1 L 263.4 67.6 L 311.1 99.0 L 324.9 120.3 L 353.7 188.1 L 360.0 223.2 L 355.0 259.6 L 312.3 308.5 L 260.9 334.9 L 214.4 337.4 L 180.5 332.4 L 134.1 333.6",
      "M 196.9 183.1 L 205.6 96.5 L 179.3 102.7 L 224.5 114.0 L 203.1 76.4 L 178.0 95.2 L 211.9 105.3 L 213.2 97.7 L 200.6 83.9 L 194.4 80.2 L 188.1 87.7 L 194.4 90.2 L 198.1 85.2"
    ],
    "tips": "Trace outline in 2 clean strokes for instant 90%+ AI recognition."
  },
  "cookie": {
    "difficulty": "medium",
    "name": "Cookie",
    "paths": [
      "M 200 70 A 130 130 0 1 0 200 330 A 130 130 0 1 0 200 70 Z",
      "M 150 150 A 10 10 0 1 0 150 151",
      "M 250 160 A 10 10 0 1 0 250 161",
      "M 180 220 A 10 10 0 1 0 180 221",
      "M 240 240 A 10 10 0 1 0 240 241",
      "M 140 250 A 10 10 0 1 0 140 251"
    ],
    "tips": "1. Draw round cookie. 2. Add chocolate chips inside."
  },
  "cruise ship": {
    "difficulty": "medium",
    "name": "Cruise Ship",
    "paths": [
      "M 60 250 L 340 250 L 300 310 L 100 310 Z",
      "M 90 250 L 90 180 L 270 180 L 285 250",
      "M 120 180 L 120 140 L 150 140 L 150 180",
      "M 170 180 L 170 140 L 200 140 L 200 180",
      "M 50 320 Q 100 330 150 320 Q 200 310 250 320 Q 300 330 350 320"
    ],
    "tips": "1. Draw large boat hull. 2. Add stepped cabin decks and funnels."
  },
  "dresser": {
    "difficulty": "medium",
    "name": "Dresser",
    "paths": [
      "M 110 110 L 290 110 L 290 310 L 110 310 Z",
      "M 110 170 L 290 170",
      "M 110 230 L 290 230",
      "M 180 140 L 190 140",
      "M 210 140 L 220 140",
      "M 180 200 L 190 200",
      "M 210 200 L 220 200",
      "M 180 260 L 190 260",
      "M 210 260 L 220 260"
    ],
    "tips": "1. Draw rectangular chest of drawers. 2. Add three horizontal drawer dividers with knobs."
  },
  "dumbbell": {
    "difficulty": "medium",
    "name": "Dumbbell",
    "paths": [
      "M 130 200 L 270 200",
      "M 100 150 L 130 150 L 130 250 L 100 250 Z",
      "M 270 150 L 300 150 L 300 250 L 270 250 Z"
    ],
    "tips": "1. Draw horizontal grip bar. 2. Add weight plates on both ends."
  },
  "ear": {
    "difficulty": "medium",
    "name": "Ear",
    "paths": [
      "M 114.7 125.3 L 118.4 99.0 L 154.8 57.6 L 181.2 43.8 L 203.8 40.0 L 246.4 41.3 L 262.7 53.8 L 275.3 73.9 L 287.8 121.6 L 285.3 176.8 L 275.3 214.4 L 240.2 282.2 L 201.3 323.6 L 197.5 334.9 L 198.7 346.2 L 181.2 358.7 L 141.0 360.0 L 113.4 343.7 L 112.2 334.9 L 119.7 313.6"
    ],
    "tips": "Trace outline in one continuous stroke for instant 90%+ AI recognition."
  },
  "eyeglasses": {
    "difficulty": "medium",
    "name": "Eyeglasses",
    "paths": [
      "M 204.4 116.5 L 233.3 122.8 L 240.8 135.4 L 230.7 117.8 L 210.7 110.3 L 179.3 119.1 L 151.7 132.9 L 40.0 213.2 L 41.3 208.2 L 80.2 189.3 L 105.3 185.6 L 121.6 191.8 L 135.4 209.4 L 142.9 232.0 L 144.2 250.8 L 135.4 269.6 L 111.5 280.9 L 70.1 278.4 L 52.5 265.9 L 47.5 220.7 L 58.8 199.4 L 92.7 185.6 L 114.0 185.6 L 125.3 189.3 L 136.6 213.2 L 142.9 243.3 L 150.4 248.3 L 210.7 248.3 L 220.7 206.9 L 248.3 194.4 L 280.9 193.1 L 311.1 205.6 L 322.4 215.7 L 324.9 248.3 L 319.8 279.7 L 308.5 289.7 L 265.9 286.0 L 224.5 273.4 L 203.1 259.6 L 195.6 247.1 L 199.4 229.5 L 211.9 215.7 L 249.6 200.6 L 283.5 199.4 L 293.5 204.4 L 307.3 219.5 L 322.4 243.3 L 323.6 254.6 L 328.6 253.3 L 360.0 156.7"
    ],
    "tips": "Trace outline in one continuous stroke for instant 90%+ AI recognition."
  },
  "fish": {
    "difficulty": "medium",
    "name": "Fish",
    "paths": [
      "M 310 150 L 270 200 C 230 150 140 150 90 200 C 140 250 230 250 270 200 L 310 250 Z"
    ],
    "tips": "Trace the streamlined fish body and triangular tail in one continuous stroke."
  },
  "flower": {
    "difficulty": "medium",
    "name": "Flower",
    "paths": [
      "M 188.0 360.0 L 189.3 310.9 L 203.1 226.5 L 201.9 205.0 L 190.6 164.7 L 145.2 164.7 L 120.0 160.9 L 112.4 155.9 L 108.7 139.5 L 125.0 106.8 L 149.0 94.2 L 175.4 86.6 L 203.1 86.6 L 233.4 95.4 L 243.5 104.3 L 248.5 123.1 L 246.0 135.7 L 212.0 159.7 L 189.3 168.5 L 174.2 162.2",
      "M 184.3 108.0 L 185.5 80.3 L 200.6 55.1 L 222.0 40.0 L 252.3 41.3 L 276.2 61.4 L 282.5 72.8 L 283.8 100.5 L 268.7 114.3 L 268.7 105.5 L 277.5 104.3 L 317.8 123.1 L 331.7 148.3 L 330.4 168.5 L 311.5 183.6 L 297.6 186.1 L 275.0 184.9 L 263.6 181.1 L 235.9 162.2 L 232.1 153.4 L 239.7 147.1 L 259.8 160.9 L 264.9 181.1 L 248.5 206.3 L 232.1 216.4 L 206.9 220.2 L 180.5 213.9 L 170.4 206.3 L 161.6 186.1 L 150.2 201.3 L 131.3 203.8 L 113.7 189.9 L 108.7 173.5 L 108.7 155.9 L 122.5 140.8 L 78.4 138.3 L 68.3 126.9 L 69.6 96.7 L 83.5 79.1 L 97.3 71.5 L 128.8 66.5 L 150.2 79.1 L 171.7 101.7"
    ],
    "tips": "Trace outline in 2 clean strokes for instant 90%+ AI recognition."
  },
  "flying saucer": {
    "difficulty": "medium",
    "name": "Flying Saucer",
    "paths": [
      "M 70 210 C 70 180 330 180 330 210 C 330 240 70 240 70 210 Z",
      "M 140 185 C 140 130 260 130 260 185",
      "M 120 240 L 100 270",
      "M 200 240 L 200 280",
      "M 280 240 L 300 270"
    ],
    "tips": "1. Draw disc saucer body. 2. Add glass cockpit dome and landing beams."
  },
  "foot": {
    "difficulty": "medium",
    "name": "Foot",
    "paths": [
      "M 173.6 40.0 L 182.4 61.3 L 188.7 99.0 L 190.0 186.8 L 169.9 195.6 L 144.8 198.1 L 131.0 193.1 L 108.4 193.1 L 92.1 203.1 L 83.3 215.7 L 83.3 238.3 L 89.6 243.3 L 127.2 244.5 L 84.5 244.5 L 68.2 267.1 L 62.0 289.7 L 69.5 296.0 L 85.8 299.8 L 118.4 301.0 L 138.5 294.7 L 142.3 288.5 L 118.4 288.5 L 93.3 311.1 L 80.8 329.9 L 77.0 356.2 L 115.9 360.0 L 162.4 338.7 L 215.1 332.4 L 275.3 333.6 L 331.8 323.6 L 338.0 314.8 L 338.0 274.7 L 319.2 229.5 L 307.9 171.8 L 307.9 142.9"
    ],
    "tips": "Trace foot silhouette from ankle down sole to toe in one continuous stroke."
  },
  "giraffe": {
    "difficulty": "medium",
    "name": "Giraffe",
    "paths": [
      "M 140 80 L 170 80 L 170 110 L 190 90 L 180 130 L 180 240 L 270 240 L 270 340 L 250 340 L 250 270 L 220 270 L 220 340 L 200 340 L 200 270 L 160 270 L 160 340 L 140 340 L 145 250 L 145 130 L 120 120 L 120 90 Z"
    ],
    "tips": "Trace the giraffe in one continuous stroke: head horns, tall long neck, back, legs."
  },
  "grapes": {
    "difficulty": "medium",
    "name": "Grapes",
    "paths": [
      "M 156.5 210.1 L 141.4 231.5 L 125.0 269.3 L 122.5 286.9 L 126.3 297.0 L 142.7 310.9 L 162.8 318.4 L 179.2 317.2 L 190.6 305.8 L 193.1 286.9 L 188.0 259.2 L 175.4 232.8 L 157.8 211.3",
      "M 230.9 283.1 L 213.2 297.0 L 206.9 305.8 L 199.4 328.5 L 201.9 341.1 L 213.2 352.4 L 234.6 360.0 L 252.3 360.0 L 264.9 353.7 L 273.7 336.1 L 269.9 323.5 L 259.8 308.3 L 234.6 293.2 L 206.9 286.9 L 222.0 285.7 L 228.3 273.1 L 240.9 227.7 L 238.4 201.3 L 227.1 198.7 L 214.5 210.1 L 203.1 227.7 L 198.1 246.6 L 199.4 264.3 L 205.7 278.1 L 220.8 289.4 L 232.1 292.0 L 256.1 285.7 L 272.4 274.3 L 277.5 266.8 L 277.5 250.4 L 269.9 231.5 L 252.3 210.1 L 235.9 206.3 L 220.8 215.1 L 218.3 220.2",
      "M 155.3 220.2 L 172.9 169.8 L 201.9 113.1",
      "M 218.3 203.8 L 222.0 179.8 L 193.1 89.1 L 196.9 72.8",
      "M 194.3 114.3 L 200.6 94.2 L 213.2 74.0 L 229.6 55.1 L 258.6 40.0",
      "M 191.8 104.3 L 191.8 104.3"
    ],
    "tips": "Trace outline in 6 clean strokes for instant 90%+ AI recognition."
  },
  "guitar": {
    "difficulty": "medium",
    "name": "Guitar",
    "paths": [
      "M 190 50 L 210 50 L 210 160 C 240 170 245 200 230 220 C 260 245 250 310 200 330 C 150 310 140 245 170 220 C 155 200 160 170 190 160 Z",
      "M 200 250 A 15 15 0 1 0 200 251"
    ],
    "tips": "1. Draw the figure-8 guitar body and straight neck in one continuous stroke. 2. Add center sound hole."
  },
  "hamburger": {
    "difficulty": "medium",
    "name": "Hamburger",
    "paths": [
      "M 53.8 208.8 L 61.3 169.9 L 92.7 129.7 L 141.6 90.8 L 174.3 79.5 L 244.5 79.5 L 272.2 92.1 L 294.7 112.2 L 312.3 134.7 L 344.9 206.3 L 287.2 211.3 L 43.8 212.5",
      "M 56.3 269.0 L 356.2 269.0 L 360.0 275.3 L 360.0 301.6 L 353.7 309.2 L 324.9 316.7 L 55.1 320.5 L 41.3 315.5 L 40.0 287.8 L 45.0 277.8"
    ],
    "tips": "Trace outline in 2 clean strokes for instant 90%+ AI recognition."
  },
  "hammer": {
    "difficulty": "medium",
    "name": "Hammer",
    "paths": [
      "M 110 100 L 270 100 L 270 160 L 110 160 Z",
      "M 175 160 L 175 340 L 205 340 L 205 160 Z"
    ],
    "tips": "1. Draw heavy steel hammer head. 2. Add long straight handle."
  },
  "hand": {
    "difficulty": "hard",
    "name": "Hand",
    "paths": [
      "M 140 320 L 140 240 C 110 230 90 200 110 180 C 130 160 150 185 160 205 L 160 120 C 160 100 180 100 180 120 L 180 200 L 190 100 C 190 80 210 80 210 100 L 210 200 L 220 120 C 220 100 240 100 240 120 L 240 205 L 250 150 C 250 130 270 130 270 150 L 265 240 L 265 320 Z"
    ],
    "tips": "Trace the 5 fingers and palm in one continuous fluid stroke from wrist to fingers and back."
  },
  "helicopter": {
    "difficulty": "medium",
    "name": "Helicopter",
    "paths": [
      "M 110 200 C 110 150 220 150 230 200 L 330 210 L 330 190 L 340 190 L 340 230 L 330 230 L 330 215 L 210 240 L 130 240 Z",
      "M 170 150 L 170 120",
      "M 90 120 L 250 120",
      "M 140 240 L 130 270",
      "M 190 240 L 200 270",
      "M 110 270 L 220 270"
    ],
    "tips": "1. Draw round cabin with tail boom. 2. Add top rotor blade and landing skids."
  },
  "hockey puck": {
    "difficulty": "medium",
    "name": "Hockey Puck",
    "paths": [
      "M 80 160 A 120 45 0 1 0 320 160 A 120 45 0 1 0 80 160 Z",
      "M 80 160 L 80 230 C 80 280 320 280 320 230 L 320 160"
    ],
    "tips": "1. Draw top round ellipse. 2. Add cylindrical 3D sides and bottom curve."
  },
  "hospital": {
    "difficulty": "medium",
    "name": "Hospital",
    "paths": [
      "M 100 130 L 300 130 L 300 320 L 100 320 Z",
      "M 180 180 L 220 180",
      "M 200 160 L 200 200",
      "M 130 220 L 160 220 L 160 250 L 130 250 Z",
      "M 240 220 L 270 220 L 270 250 L 240 250 Z",
      "M 180 270 L 220 270 L 220 320 L 180 320 Z"
    ],
    "tips": "1. Draw hospital building. 2. Add medical cross sign on front."
  },
  "key": {
    "difficulty": "medium",
    "name": "Key",
    "paths": [
      "M 110 200 A 35 35 0 1 0 110 201",
      "M 110 200 A 15 15 0 1 0 110 201",
      "M 145 200 L 290 200",
      "M 270 200 L 270 230",
      "M 290 200 L 290 240"
    ],
    "tips": "1. Draw round ring bow. 2. Add long stem shaft with teeth."
  },
  "knee": {
    "difficulty": "medium",
    "name": "Knee",
    "paths": [
      "M 80 150 L 220 150 C 270 150 280 180 280 210 L 240 340",
      "M 80 190 L 180 190 C 215 190 225 210 200 340"
    ],
    "tips": "Draw the bent knee profile showing thigh, rounded kneecap, and lower leg."
  },
  "lantern": {
    "difficulty": "medium",
    "name": "Lantern",
    "paths": [
      "M 122.5 166.0 L 128.8 197.5 L 130.1 293.2 L 136.4 295.7 L 286.3 295.7 L 283.8 268.0 L 283.8 171.0 L 280.0 160.9 L 269.9 149.6 L 240.9 149.6 L 127.6 163.5 L 126.3 155.9 L 130.1 99.2 L 140.2 82.8 L 141.4 75.3 L 161.6 51.3 L 179.2 40.0 L 223.3 40.0 L 233.4 45.0 L 258.6 66.5 L 272.4 94.2 L 276.2 145.8",
      "M 133.9 285.7 L 121.3 300.8 L 121.3 308.3 L 73.4 343.6 L 88.5 347.4 L 242.2 360.0 L 326.6 360.0 L 298.9 300.8 L 275.0 265.5",
      "M 159.1 160.9 L 162.8 303.3"
    ],
    "tips": "Trace outline in 3 clean strokes for instant 90%+ AI recognition."
  },
  "laptop": {
    "difficulty": "medium",
    "name": "Laptop",
    "paths": [
      "M 130 110 L 270 110 L 270 230 L 130 230 Z",
      "M 145 125 L 255 125 L 255 215 L 145 215 Z",
      "M 130 230 L 90 290 L 310 290 L 270 230 Z"
    ],
    "tips": "1. Draw rectangular display screen. 2. Add angled keyboard base at bottom."
  },
  "leaf": {
    "difficulty": "medium",
    "name": "Leaf",
    "paths": [
      "M 218.2 360.0 L 211.9 292.2",
      "M 211.9 292.2 L 199.4 292.2 L 152.9 268.4 L 129.1 242.0 L 121.6 209.4 L 121.6 186.8 L 125.3 166.7 L 137.9 141.6 L 183.1 81.4 L 199.4 40.0 L 206.9 92.7 L 250.8 139.1 L 270.9 193.1 L 278.4 252.1 L 272.2 268.4 L 262.1 278.4 L 239.5 289.7 L 211.9 292.2 L 199.4 298.5 L 174.3 211.9 L 152.9 109.0",
      "M 195.6 229.5 L 195.6 229.5"
    ],
    "tips": "Trace outline in 3 clean strokes for instant 90%+ AI recognition."
  },
  "lighter": {
    "difficulty": "medium",
    "name": "Lighter",
    "paths": [
      "M 150 160 L 230 160 L 230 330 L 150 330 Z",
      "M 150 160 L 110 120 L 130 100 L 170 140",
      "M 190 160 L 190 125 L 220 125 L 220 160",
      "M 205 125 Q 195 90 205 70 Q 215 90 205 125 Z"
    ],
    "tips": "1. Draw rectangular case body. 2. Add metal hood spark wheel and flame."
  },
  "lollipop": {
    "difficulty": "medium",
    "name": "Lollipop",
    "paths": [
      "M 200 80 A 65 65 0 1 0 200 210 A 65 65 0 1 0 200 80 Z",
      "M 200 145 C 220 135 220 160 200 160 C 180 160 180 130 200 125",
      "M 200 210 L 200 340"
    ],
    "tips": "1. Draw round candy disc. 2. Add straight vertical stick at base."
  },
  "mailbox": {
    "difficulty": "medium",
    "name": "Mailbox",
    "paths": [
      "M 130 210 L 130 150 C 130 100 270 100 270 150 L 270 210 Z",
      "M 190 210 L 190 340 L 210 340 L 210 210",
      "M 270 150 L 300 150 L 300 120 L 320 120 L 320 135 L 300 135"
    ],
    "tips": "1. Draw arched mailbox. 2. Add vertical post and red flag."
  },
  "mermaid": {
    "difficulty": "medium",
    "name": "Mermaid",
    "paths": [
      "M 200 80 A 15 15 0 1 0 200 110 A 15 15 0 1 0 200 80 Z",
      "M 185 110 L 185 170 L 215 170 L 215 110",
      "M 185 170 C 170 230 240 260 220 310 L 190 340",
      "M 220 310 L 250 340"
    ],
    "tips": "1. Draw head and arms. 2. Add long curved fish tail with fluke fins."
  },
  "microphone": {
    "difficulty": "medium",
    "name": "Microphone",
    "paths": [
      "M 160 140 C 160 90 240 90 240 140 C 240 180 160 180 160 140 Z",
      "M 160 140 L 240 140",
      "M 180 180 L 180 320 L 220 320 L 220 180 Z"
    ],
    "tips": "1. Draw rounded microphone capsule. 2. Add handle shaft below."
  },
  "moon": {
    "difficulty": "medium",
    "name": "Moon",
    "paths": [
      "M 290 50 C 70 70 60 330 290 350 C 150 280 150 120 290 50 Z"
    ],
    "tips": "Trace thick crescent moon in one fluid continuous stroke."
  },
  "mouse": {
    "difficulty": "medium",
    "name": "Mouse",
    "paths": [
      "M 150 130 C 150 90 250 90 250 130 L 250 250 C 250 290 150 290 150 250 Z",
      "M 200 90 L 200 180",
      "M 150 180 L 250 180",
      "M 200 90 C 200 50 170 50 170 30"
    ],
    "tips": "1. Draw rounded computer mouse body. 2. Add click buttons and USB cord."
  },
  "mouth": {
    "difficulty": "medium",
    "name": "Mouth",
    "paths": [
      "M 40.0 200.6 L 72.6 181.8 L 102.7 142.9 L 125.3 129.1 L 159.2 126.6 L 183.1 149.2 L 201.9 156.7 L 216.9 156.7 L 249.6 144.2 L 275.9 144.2 L 317.3 185.6 L 331.1 193.1 L 360.0 194.4 L 260.9 255.8 L 233.3 268.4 L 205.6 273.4 L 184.3 273.4 L 168.0 268.4 L 116.5 245.8 L 52.5 209.4 L 68.9 204.4 L 159.2 204.4 L 216.9 195.6 L 352.5 191.8"
    ],
    "tips": "Trace outline in one continuous stroke for instant 90%+ AI recognition."
  },
  "mushroom": {
    "difficulty": "medium",
    "name": "Mushroom",
    "paths": [
      "M 100 210 C 100 100 300 100 300 210 Z",
      "M 170 210 L 170 330 L 230 330 L 230 210"
    ],
    "tips": "1. Draw domed umbrella cap. 2. Add thick stalk underneath."
  },
  "nose": {
    "difficulty": "medium",
    "name": "Nose",
    "paths": [
      "M 200 90 L 185 240 C 185 265 215 265 215 240",
      "M 165 235 C 165 255 185 255 185 240",
      "M 215 240 C 215 255 235 255 235 235"
    ],
    "tips": "1. Draw nasal bridge line. 2. Add rounded tip and nostrils."
  },
  "paper clip": {
    "difficulty": "medium",
    "name": "Paper Clip",
    "paths": [
      "M 160 300 L 160 140 C 160 100 240 100 240 140 L 240 280 C 240 310 180 310 180 280 L 180 170 C 180 150 220 150 220 170 L 220 260"
    ],
    "tips": "1. Draw metallic wire bending in overlapping loops."
  },
  "parachute": {
    "difficulty": "medium",
    "name": "Parachute",
    "paths": [
      "M 100 170 C 100 90 300 90 300 170 C 270 160 230 160 200 170 C 170 160 130 160 100 170 Z",
      "M 100 170 L 200 300",
      "M 150 165 L 200 300",
      "M 250 165 L 200 300",
      "M 300 170 L 200 300"
    ],
    "tips": "1. Draw arched canopy. 2. Add lines converging down to parachutist."
  },
  "peanut": {
    "difficulty": "medium",
    "name": "Peanut",
    "paths": [
      "M 92.7 184.3 L 65.1 194.4 L 53.8 206.9 L 46.3 222.0 L 41.3 244.5 L 40.0 262.1 L 42.5 270.9 L 55.1 286.0 L 80.2 307.3 L 115.3 322.4 L 161.7 327.4 L 174.3 323.6 L 190.6 302.3 L 198.1 288.5 L 206.9 252.1 L 229.5 223.2 L 244.5 216.9 L 293.5 211.9 L 313.6 203.1 L 342.4 176.8 L 350.0 165.5 L 358.7 139.1 L 360.0 121.6 L 355.0 107.8 L 339.9 90.2 L 319.8 81.4 L 269.6 72.6 L 232.0 73.9 L 222.0 77.6 L 214.4 86.4 L 191.8 134.1 L 174.3 152.9 L 127.8 174.3 L 88.9 179.3 L 72.6 185.6"
    ],
    "tips": "Trace outline in one continuous stroke for instant 90%+ AI recognition."
  },
  "pear": {
    "difficulty": "medium",
    "name": "Pear",
    "paths": [
      "M 200 120 C 160 120 160 170 130 210 C 100 250 120 320 200 320 C 280 320 300 250 270 210 C 240 170 240 120 200 120 Z",
      "M 200 120 Q 210 80 225 65"
    ],
    "tips": "1. Draw narrow rounded top expanding to wide bottom. 2. Add stem."
  },
  "peas": {
    "difficulty": "medium",
    "name": "Peas",
    "paths": [
      "M 149.2 222.0 L 140.4 227.0 L 136.6 242.0 L 146.7 252.1 L 156.7 253.3 L 166.7 249.6 L 173.0 245.8 L 174.3 237.0 L 151.7 220.7",
      "M 239.5 220.7 L 227.0 243.3 L 225.7 257.1 L 230.7 263.4 L 239.5 267.1 L 250.8 265.9 L 263.4 254.6 L 265.9 243.3 L 263.4 237.0 L 248.3 232.0",
      "M 223.2 141.6 L 240.8 132.9 L 257.1 136.6 L 258.4 145.4 L 254.6 151.7 L 238.3 160.5 L 228.2 160.5 L 223.2 151.7 L 227.0 145.4",
      "M 351.2 166.7 L 338.7 176.8 L 336.2 184.3 L 336.2 190.6 L 344.9 194.4 L 353.7 190.6 L 360.0 176.8 L 356.2 170.5 L 347.5 165.5",
      "M 120.3 136.6 L 114.0 145.4 L 120.3 159.2 L 134.1 160.5 L 147.9 151.7 L 147.9 142.9 L 129.1 139.1",
      "M 45.0 201.9 L 40.0 219.5 L 51.3 232.0 L 61.3 230.7 L 73.9 220.7 L 62.6 209.4"
    ],
    "tips": "Trace outline in 6 clean strokes for instant 90%+ AI recognition."
  },
  "pineapple": {
    "difficulty": "medium",
    "name": "Pineapple",
    "paths": [
      "M 140 170 C 130 220 130 270 160 305 C 180 320 220 320 240 305 C 270 270 270 220 260 170 C 240 150 160 150 140 170 Z",
      "M 200 150 L 170 80 L 195 100 L 200 70 L 205 100 L 230 80 L 200 150",
      "M 145 200 L 255 270",
      "M 145 240 L 235 300",
      "M 255 200 L 145 270",
      "M 255 240 L 165 300"
    ],
    "tips": "1. Draw oval fruit with diamond criss-cross texture. 2. Add spiky leafy crown."
  },
  "pizza": {
    "difficulty": "medium",
    "name": "Pizza",
    "paths": [
      "M 200 320 L 120 110 Q 200 80 280 110 Z",
      "M 130 120 Q 200 95 270 120",
      "M 180 160 A 10 10 0 1 0 180 161",
      "M 220 190 A 10 10 0 1 0 220 191",
      "M 190 240 A 10 10 0 1 0 190 241"
    ],
    "tips": "1. Draw triangular slice with crust arc. 2. Add round pepperoni toppings."
  },
  "pond": {
    "difficulty": "medium",
    "name": "Pond",
    "paths": [
      "M 347.5 139.1 L 282.2 120.3 L 235.8 116.5 L 184.3 117.8 L 134.1 134.1 L 92.7 156.7 L 63.8 188.1 L 46.3 215.7 L 40.0 240.8 L 40.0 260.9 L 52.5 273.4 L 83.9 283.5 L 175.5 282.2 L 223.2 270.9 L 306.0 239.5 L 351.2 210.7 L 360.0 196.9 L 356.2 175.5 L 292.2 127.8 L 264.6 121.6",
      "M 85.2 232.0 L 111.5 210.7 L 140.4 168.0 L 147.9 166.7 L 166.7 185.6 L 181.8 191.8 L 194.4 190.6 L 210.7 161.7 L 229.5 176.8 L 245.8 175.5 L 263.4 161.7 L 273.4 171.8 L 288.5 176.8"
    ],
    "tips": "Trace outline in 2 clean strokes for instant 90%+ AI recognition."
  },
  "popsicle": {
    "difficulty": "medium",
    "name": "Popsicle",
    "paths": [
      "M 150 140 C 150 90 250 90 250 140 L 250 260 L 150 260 Z",
      "M 200 120 L 200 240",
      "M 185 260 L 185 330 L 215 330 L 215 260"
    ],
    "tips": "1. Draw rounded ice pop bar. 2. Add wooden stick at bottom."
  },
  "power outlet": {
    "difficulty": "medium",
    "name": "Power Outlet",
    "paths": [
      "M 120 100 L 280 100 C 300 100 300 120 300 140 L 300 260 C 300 280 280 300 260 300 L 140 300 C 120 300 100 280 100 260 L 100 140 C 100 120 120 100 120 100 Z",
      "M 160 150 L 160 190",
      "M 240 150 L 240 190",
      "M 200 220 A 10 10 0 1 0 200 240 A 10 10 0 1 0 200 220 Z"
    ],
    "tips": "1. Draw wall plate. 2. Add twin vertical slots and round ground pin."
  },
  "remote control": {
    "difficulty": "medium",
    "name": "Remote Control",
    "paths": [
      "M 149.2 48.8 L 146.7 86.4 L 146.7 341.2 L 173.0 350.0 L 247.1 360.0 L 245.8 170.5 L 248.3 77.6 L 253.3 42.5 L 247.1 40.0 L 149.2 40.0",
      "M 161.7 53.8 L 159.2 81.4 L 179.3 87.7 L 181.8 83.9 L 181.8 53.8 L 165.5 53.8"
    ],
    "tips": "Trace outline in 2 clean strokes for instant 90%+ AI recognition."
  },
  "roller coaster": {
    "difficulty": "medium",
    "name": "Roller Coaster",
    "paths": [
      "M 60 280 Q 130 80 180 200 Q 230 300 280 140 L 340 280",
      "M 130 130 L 130 330",
      "M 180 200 L 180 330",
      "M 280 140 L 280 330"
    ],
    "tips": "1. Draw undulating track peaks and valleys. 2. Add tall vertical support scaffolding."
  },
  "sandwich": {
    "difficulty": "medium",
    "name": "Sandwich",
    "paths": [
      "M 90 250 L 310 250 L 200 120 Z",
      "M 90 270 L 310 270 L 310 250 L 90 250 Z",
      "M 100 250 Q 150 260 200 250 Q 250 260 300 250"
    ],
    "tips": "1. Draw diagonal bread triangle slices. 2. Add wavy filling layers."
  },
  "saxophone": {
    "difficulty": "medium",
    "name": "Saxophone",
    "paths": [
      "M 140 80 L 180 80 L 170 130 L 170 260 C 170 310 240 310 250 250 L 280 200 L 240 180 L 225 240 C 220 270 195 270 195 240 L 195 130 Z"
    ],
    "tips": "Trace the curved saxophone body and upward-curving bell in one continuous stroke."
  },
  "screwdriver": {
    "difficulty": "medium",
    "name": "Screwdriver",
    "paths": [
      "M 180 80 L 220 80 L 220 180 L 180 180 Z",
      "M 195 180 L 195 320 L 190 330 L 210 330 L 205 320 L 205 180"
    ],
    "tips": "1. Draw fluted grip handle. 2. Add straight metal shaft and flat blade tip."
  },
  "shoe": {
    "difficulty": "medium",
    "name": "Shoe",
    "paths": [
      "M 220 90 L 335 90 L 355 250 L 350 285 L 235 315 L 95 315 L 65 300 L 40 275 L 40 240 L 90 170 L 140 150 L 175 150 L 220 175 L 220 90 Z"
    ],
    "tips": "Trace the boot/shoe silhouette from ankle collar down to toe and sole in one continuous stroke."
  },
  "shovel": {
    "difficulty": "medium",
    "name": "Shovel",
    "paths": [
      "M 180 60 L 220 60 L 220 90 L 180 90 Z",
      "M 195 90 L 195 240 L 205 240 L 205 90",
      "M 160 240 L 240 240 L 230 320 L 200 340 L 170 320 Z"
    ],
    "tips": "1. Draw top D-grip handle. 2. Add long shaft and scoop blade."
  },
  "sink": {
    "difficulty": "medium",
    "name": "Sink",
    "paths": [
      "M 104.0 213.8 L 92.7 231.4 L 55.1 269.0 L 41.3 287.8 L 40.0 292.9 L 48.8 297.9 L 82.7 302.9 L 149.2 304.2 L 357.5 292.9 L 360.0 287.8 L 342.4 242.7 L 332.4 191.2 L 324.9 186.2 L 308.5 191.2 L 265.9 196.2 L 176.8 201.3 L 97.7 200.0",
      "M 222.0 188.7 L 224.5 139.8 L 216.9 109.6 L 209.4 104.6 L 199.4 108.4 L 191.8 118.4 L 189.3 139.8 L 180.5 141.0 L 179.3 132.2 L 185.6 107.1 L 191.8 102.1 L 211.9 95.8 L 229.5 95.8 L 237.0 103.4 L 242.0 123.5 L 243.3 172.4 L 239.5 192.5"
    ],
    "tips": "Trace outline in 2 clean strokes for instant 90%+ AI recognition."
  },
  "soccer ball": {
    "difficulty": "medium",
    "name": "Soccer Ball",
    "paths": [
      "M 200 80 A 120 120 0 1 0 200 320 A 120 120 0 1 0 200 80 Z",
      "M 200 145 L 252 184 L 232 244 L 168 244 L 148 184 Z",
      "M 200 145 L 200 80",
      "M 252 184 L 316 160",
      "M 232 244 L 275 305",
      "M 168 244 L 125 305",
      "M 148 184 L 84 160"
    ],
    "tips": "1. Draw outer circle. 2. Add center pentagon patch and lines radiating to edges."
  },
  "sock": {
    "difficulty": "medium",
    "name": "Sock",
    "paths": [
      "M 150 100 L 220 100 L 220 220 C 250 220 300 230 300 270 C 300 300 260 310 230 300 L 150 250 C 130 240 140 210 150 200 Z",
      "M 150 120 L 220 120"
    ],
    "tips": "1. Draw upright ankle tube. 2. Turn at heel and curve into rounded toe."
  },
  "stethoscope": {
    "difficulty": "medium",
    "name": "Stethoscope",
    "paths": [
      "M 61.3 43.1 L 46.3 50.7 L 40.0 65.7 L 40.0 88.3 L 62.6 138.5 L 94.0 169.9 L 132.9 198.7 L 159.2 215.1 L 209.4 232.6 L 214.4 240.2 L 218.2 315.5 L 210.7 320.5 L 170.5 314.2 L 164.2 329.3 L 166.7 344.3 L 183.1 354.4 L 196.9 356.9 L 228.2 356.9 L 243.3 350.6 L 249.6 343.1 L 250.8 326.7 L 245.8 320.5 L 213.2 301.6",
      "M 214.4 232.6 L 230.7 240.2 L 253.3 242.7 L 293.5 223.8 L 312.3 210.0 L 344.9 163.6 L 355.0 141.0 L 360.0 82.0 L 346.2 60.7 L 316.1 45.6 L 306.0 45.6"
    ],
    "tips": "Trace outline in 2 clean strokes for instant 90%+ AI recognition."
  },
  "stop sign": {
    "difficulty": "medium",
    "name": "Stop Sign",
    "paths": [
      "M 140 80 L 260 80 L 320 180 L 260 280 L 140 280 L 80 180 Z",
      "M 149 129 L 251 231",
      "M 251 129 L 149 231",
      "M 200 280 L 200 370"
    ],
    "tips": "1. Draw hexagon perimeter. 2. Add X mark inside. 3. Add vertical support pole."
  },
  "strawberry": {
    "difficulty": "medium",
    "name": "Strawberry",
    "paths": [
      "M 200 130 C 150 110 110 170 130 230 C 150 280 190 320 200 330 C 210 320 250 280 270 230 C 290 170 250 110 200 130 Z",
      "M 200 130 L 170 90 L 190 110 L 200 80 L 210 110 L 230 90 Z",
      "M 170 170 A 3 3 0 1 0 170 171",
      "M 230 170 A 3 3 0 1 0 230 171",
      "M 200 210 A 3 3 0 1 0 200 211"
    ],
    "tips": "1. Draw heart-shaped berry. 2. Add leafy calyx cap at top. 3. Add seed dots."
  },
  "streetlight": {
    "difficulty": "medium",
    "name": "Streetlight",
    "paths": [
      "M 278.4 360.0 L 272.2 341.2 L 272.2 260.9 L 265.9 211.9 L 264.6 114.0 L 255.8 46.3 L 242.0 40.0 L 188.1 40.0 L 161.7 47.5 L 146.7 63.8 L 136.6 86.4 L 134.1 100.2 L 150.4 101.5 L 145.4 117.8 L 134.1 119.1 L 122.8 115.3 L 121.6 99.0 L 130.4 99.0"
    ],
    "tips": "Trace outline in one continuous stroke for instant 90%+ AI recognition."
  },
  "suitcase": {
    "difficulty": "medium",
    "name": "Suitcase",
    "paths": [
      "M 46.3 304.6 L 40.0 235.3 L 43.8 160.9 L 62.7 155.9 L 147.1 149.6 L 360.0 144.6 L 355.0 203.8 L 360.0 285.7 L 299.5 293.2 L 210.1 293.2 L 55.1 305.8",
      "M 138.3 153.4 L 135.7 138.3 L 144.6 118.1 L 191.2 94.2 L 201.3 94.2 L 208.8 99.2 L 223.9 123.1 L 226.5 138.3"
    ],
    "tips": "Trace outline in 2 clean strokes for instant 90%+ AI recognition."
  },
  "syringe": {
    "difficulty": "medium",
    "name": "Syringe",
    "paths": [
      "M 140 170 L 270 170 L 270 230 L 140 230 Z",
      "M 270 200 L 320 200",
      "M 320 180 L 320 220",
      "M 140 200 L 70 200",
      "M 170 170 L 170 190",
      "M 200 170 L 200 190",
      "M 230 170 L 230 190"
    ],
    "tips": "1. Draw barrel with dosage lines. 2. Add plunger at rear and fine needle at front."
  },
  "table": {
    "difficulty": "medium",
    "name": "Table",
    "paths": [
      "M 80 180 L 320 180 L 320 210 L 80 210 Z",
      "M 100 210 L 100 320",
      "M 130 210 L 130 300",
      "M 270 210 L 270 300",
      "M 300 210 L 300 320"
    ],
    "tips": "1. Draw flat horizontal tabletop slab. 2. Add four vertical legs."
  },
  "teapot": {
    "difficulty": "medium",
    "name": "Teapot",
    "paths": [
      "M 140 180 C 110 210 110 270 160 300 L 240 300 C 290 270 290 210 260 180 Z",
      "M 160 180 C 160 150 240 150 240 180",
      "M 200 150 A 6 6 0 1 0 200 151",
      "M 130 220 C 80 200 90 150 110 160",
      "M 265 200 C 310 200 310 270 255 270"
    ],
    "tips": "1. Draw kettle body with domed lid. 2. Add curved spout and handle."
  },
  "teddy-bear": {
    "difficulty": "medium",
    "name": "Teddy-Bear",
    "paths": [
      "M 200 70 A 45 45 0 1 0 200 160 A 45 45 0 1 0 200 70 Z",
      "M 200 155 A 75 75 0 1 0 200 305 A 75 75 0 1 0 200 155 Z",
      "M 155 80 A 18 18 0 1 0 155 81",
      "M 245 80 A 18 18 0 1 0 245 81"
    ],
    "tips": "1. Draw snowman-style head and body circles. 2. Add rounded ears on head."
  },
  "tennis racquet": {
    "difficulty": "medium",
    "name": "Tennis Racquet",
    "paths": [
      "M 227.6 347.5 L 221.3 274.7 L 205.0 224.5 L 183.7 206.9 L 105.9 158.0 L 95.8 141.6 L 88.3 117.8 L 89.6 92.7 L 100.9 78.9 L 132.2 56.3 L 151.1 47.5 L 191.2 40.0 L 222.6 41.3 L 261.5 47.5 L 280.3 55.1 L 304.2 71.4 L 311.7 83.9 L 311.7 95.2 L 286.6 139.1 L 221.3 194.4 L 227.6 235.8 L 236.4 257.1 L 243.9 294.7 L 254.0 314.8 L 261.5 351.2 L 257.7 357.5 L 243.9 360.0 L 230.1 353.7 L 222.6 342.4",
      "M 99.6 86.4 L 197.5 70.1 L 280.3 63.8",
      "M 301.6 87.7 L 231.4 90.2 L 173.6 116.5 L 98.4 135.4",
      "M 112.2 154.2 L 129.7 154.2 L 235.1 129.1 L 282.8 121.6",
      "M 160 50 L 160 170",
      "M 230 45 L 230 175"
    ],
    "tips": "1. Draw frame and handle. 2. Cross horizontal and vertical strings inside head."
  },
  "tent": {
    "difficulty": "medium",
    "name": "Tent",
    "paths": [
      "M 170 140 L 90 300 L 250 300 Z",
      "M 170 140 L 170 300",
      "M 170 140 L 310 200 L 330 280 L 250 300"
    ],
    "tips": "1. Draw triangular front door flap. 2. Add ridge line angled to back stake."
  },
  "traffic light": {
    "difficulty": "medium",
    "name": "Traffic Light",
    "paths": [
      "M 160 90 L 240 90 L 240 310 L 160 310 Z",
      "M 200 115 A 20 20 0 1 0 200 155 A 20 20 0 1 0 200 115 Z",
      "M 200 180 A 20 20 0 1 0 200 220 A 20 20 0 1 0 200 180 Z",
      "M 200 245 A 20 20 0 1 0 200 285 A 20 20 0 1 0 200 245 Z"
    ],
    "tips": "1. Draw tall rectangular housing. 2. Add three stacked circle lights."
  },
  "trumpet": {
    "difficulty": "medium",
    "name": "Trumpet",
    "paths": [
      "M 80 190 L 280 190 L 330 140 L 330 260 L 280 210 L 80 210 Z",
      "M 170 160 L 170 190",
      "M 190 160 L 190 190",
      "M 210 160 L 210 190"
    ],
    "tips": "1. Draw brass tubing. 2. Add three valve keys and flared bell."
  },
  "vase": {
    "difficulty": "medium",
    "name": "Vase",
    "paths": [
      "M 160 110 L 240 110 L 220 160 C 270 190 270 270 230 310 L 170 310 C 130 270 130 190 180 160 Z"
    ],
    "tips": "1. Draw neck opening. 2. Swell out into bulbous body with flat base."
  },
  "watermelon": {
    "difficulty": "medium",
    "name": "Watermelon",
    "paths": [
      "M 80 180 L 320 180 C 320 290 80 290 80 180 Z",
      "M 95 195 C 95 275 305 275 305 195",
      "M 150 210 A 3 3 0 1 0 150 211",
      "M 200 230 A 3 3 0 1 0 200 231",
      "M 250 210 A 3 3 0 1 0 250 211"
    ],
    "tips": "1. Draw semicircle wedge. 2. Add rind contour and seed dots."
  },
  "whale": {
    "difficulty": "hard",
    "name": "Whale",
    "paths": [
      "M 80 210 C 80 140 230 130 280 200 C 310 200 330 170 340 150 L 330 210 L 340 250 C 320 230 300 210 280 210 C 230 270 120 270 80 210 Z",
      "M 160 135 C 160 90 140 80 130 90",
      "M 160 135 C 160 90 180 80 190 90"
    ],
    "tips": "1. Draw large whale body with rear tail fluke. 2. Add water spout rising on top."
  },
  "wine bottle": {
    "difficulty": "medium",
    "name": "Wine Bottle",
    "paths": [
      "M 185 80 L 215 80 L 215 140 C 240 160 240 180 240 200 L 240 330 L 160 330 L 160 200 C 160 180 160 160 185 140 Z",
      "M 175 220 L 225 220 L 225 280 L 175 280 Z"
    ],
    "tips": "1. Draw narrow neck with cork. 2. Slope shoulders into cylindrical body and label."
  },
  "wine glass": {
    "difficulty": "medium",
    "name": "Wine Glass",
    "paths": [
      "M 150 110 L 250 110 C 250 200 150 200 150 110 Z",
      "M 200 180 L 200 300",
      "M 160 300 L 240 300"
    ],
    "tips": "1. Draw rounded goblet bowl. 2. Add slender vertical stem and base disc."
  },
  "yoga": {
    "difficulty": "medium",
    "name": "Stick Man",
    "paths": [
      "M 200 60 A 25 25 0 1 0 200 110 A 25 25 0 1 0 200 60 Z",
      "M 200 110 L 200 230",
      "M 110 150 L 290 150",
      "M 200 230 L 140 330",
      "M 200 230 L 260 330"
    ],
    "tips": "Draw stick man: circle head, straight body spine, horizontal arms, and angled legs."
  },
  "stick man": {
    "difficulty": "medium",
    "name": "Stick Man",
    "paths": [
      "M 200 60 A 25 25 0 1 0 200 110 A 25 25 0 1 0 200 60 Z",
      "M 200 110 L 200 230",
      "M 110 150 L 290 150",
      "M 200 230 L 140 330",
      "M 200 230 L 260 330"
    ],
    "tips": "Draw stick man: circle head, straight body spine, horizontal arms, and angled legs."
  },
  "airplane": {
    "difficulty": "hard",
    "name": "Airplane",
    "paths": [
      "M 70 210 L 290 210 C 320 210 330 190 290 190 L 110 190 L 80 130 L 60 130 L 80 190 L 60 210 Z",
      "M 190 210 L 170 260 L 210 260 L 230 210",
      "M 190 190 L 170 140 L 210 140 L 230 190"
    ],
    "tips": "1. Draw long fuselage tube. 2. Add sweep-back main wings. 3. Add tail fin elevators."
  },
  "ant": {
    "difficulty": "hard",
    "name": "Ant",
    "paths": [
      "M 110.3 202.5 L 91.5 218.8 L 80.2 238.9 L 77.6 265.3 L 81.4 272.8",
      "M 146.7 226.4 L 150.4 243.9 L 165.5 255.2",
      "M 203.1 203.8 L 199.4 211.3 L 205.6 223.8 L 224.5 246.4 L 247.1 262.7",
      "M 107.8 202.5 L 60.1 207.5 L 42.5 191.2 L 40.0 156.1 L 51.3 141.0 L 67.6 129.7 L 85.2 127.2 L 109.0 131.0 L 134.1 138.5 L 146.7 149.8 L 147.9 171.1 L 137.9 188.7 L 111.5 210.0 L 99.0 206.3",
      "M 129.1 191.2 L 150.4 201.3 L 176.8 205.0 L 227.0 205.0 L 247.1 197.5 L 252.1 191.2 L 252.1 171.1 L 243.3 156.1 L 228.2 144.8 L 174.3 139.8 L 151.7 146.0 L 152.9 148.5",
      "M 254.6 187.5 L 267.1 197.5 L 279.7 201.3 L 331.1 203.8 L 342.4 202.5 L 360.0 187.5"
    ],
    "tips": "Trace outline in 6 clean strokes for instant 90%+ AI recognition."
  },
  "bat": {
    "difficulty": "hard",
    "name": "Bat",
    "paths": [
      "M 185 170 L 185 250 C 185 270 215 270 215 250 L 215 170 Z",
      "M 190 170 L 180 130 L 195 150",
      "M 210 170 L 220 130 L 205 150",
      "M 185 180 C 140 120 90 140 60 170 C 80 200 110 200 130 230 C 150 220 170 230 185 240",
      "M 215 180 C 260 120 310 140 340 170 C 320 200 290 200 270 230 C 250 220 230 230 215 240"
    ],
    "tips": "1. Draw furry body with pointed ears. 2. Add scalloped webbed leathery wings spreading wide."
  },
  "blackberry": {
    "difficulty": "hard",
    "name": "Blackberry",
    "paths": [
      "M 256.5 250.8 L 256.5 235.8 L 241.4 218.2 L 226.4 209.4 L 206.3 205.6 L 163.6 206.9 L 131.0 235.8 L 118.4 268.4 L 118.4 304.8 L 124.7 321.1 L 142.3 339.9 L 163.6 353.7 L 216.3 360.0 L 231.4 351.2 L 242.7 334.9 L 254.0 308.5 L 260.2 242.0 L 257.7 228.2 L 235.1 208.2 L 196.2 209.4 L 174.9 227.0 L 157.3 248.3 L 144.8 280.9 L 142.3 317.3 L 148.5 327.4 L 173.6 341.2 L 217.6 339.9 L 227.6 329.9 L 241.4 299.8 L 241.4 257.1 L 226.4 239.5 L 200.0 230.7 L 168.6 235.8 L 152.3 260.9 L 151.1 277.2 L 151.1 299.8 L 156.1 311.1 L 186.2 323.6 L 210.0 321.1 L 236.4 286.0 L 238.9 240.8 L 223.8 224.5 L 193.7 227.0 L 181.2 239.5 L 163.6 265.9 L 157.3 294.7 L 157.3 316.1 L 173.6 329.9 L 220.1 331.1 L 236.4 314.8 L 241.4 304.8 L 241.4 264.6 L 221.3 239.5 L 202.5 229.5 L 171.1 227.0 L 158.6 232.0 L 142.3 250.8 L 133.5 289.7 L 136.0 321.1 L 151.1 327.4 L 178.7 329.9 L 195.0 323.6 L 208.8 288.5 L 208.8 257.1 L 203.8 242.0 L 193.7 233.3 L 182.4 230.7 L 169.9 230.7 L 159.8 239.5 L 154.8 258.4 L 154.8 291.0 L 158.6 303.5 L 169.9 312.3 L 193.7 314.8 L 205.0 309.8 L 215.1 296.0 L 223.8 273.4 L 227.6 237.0 L 217.6 248.3 L 212.5 260.9 L 211.3 321.1 L 218.8 294.7 L 218.8 255.8 L 208.8 252.1 L 198.7 270.9 L 191.2 306.0 L 196.2 264.6 L 184.9 268.4 L 174.9 312.3 L 188.7 284.7 L 191.2 296.0 L 178.7 317.3 L 167.4 316.1 L 166.1 277.2 L 181.2 263.4 L 191.2 260.9 L 202.5 265.9 L 206.3 307.3 L 196.2 316.1 L 184.9 314.8 L 176.2 296.0 L 172.4 253.3 L 182.4 265.9 L 182.4 293.5 L 172.4 302.3 L 172.4 257.1 L 184.9 250.8 L 212.5 254.6 L 221.3 268.4 L 227.6 308.5 L 203.8 308.5 L 159.8 291.0 L 148.5 280.9 L 148.5 267.1 L 153.6 254.6 L 167.4 245.8 L 218.8 245.8 L 232.6 257.1 L 236.4 282.2 L 233.9 293.5 L 211.3 297.3 L 206.3 284.7 L 203.8 154.2 L 208.8 125.3 L 227.6 87.7 L 260.2 40.0",
      "M 206.3 119.1 L 177.4 116.5 L 143.5 106.5 L 120.9 91.5 L 120.9 70.1 L 128.5 56.3 L 138.5 52.5 L 184.9 55.1 L 197.5 67.6 L 210.0 122.8 L 217.6 130.4",
      "M 203.8 267.1 L 191.2 239.5 L 162.4 227.0 L 139.8 227.0 L 128.5 233.3 L 118.4 253.3 L 118.4 282.2 L 123.5 299.8 L 159.8 344.9 L 191.2 351.2 L 202.5 342.4 L 227.6 287.2 L 238.9 248.3 L 238.9 222.0 L 222.6 211.9 L 181.2 211.9 L 167.4 220.7 L 152.3 239.5 L 148.5 253.3 L 146.0 284.7 L 151.1 299.8 L 166.1 321.1 L 205.0 344.9 L 255.2 351.2 L 266.5 344.9 L 277.8 332.4 L 281.6 312.3 L 281.6 272.2 L 279.1 250.8 L 270.3 233.3 L 247.7 205.6 L 232.6 199.4 L 198.7 196.9 L 162.4 218.2 L 137.3 248.3 L 133.5 293.5 L 277.8 282.2 L 266.5 245.8 L 241.4 227.0 L 218.8 222.0 L 178.7 224.5 L 162.4 234.5 L 144.8 255.8 L 138.5 270.9 L 137.3 299.8 L 148.5 314.8 L 181.2 338.7 L 200.0 344.9 L 255.2 347.5 L 277.8 324.9 L 276.5 291.0 L 255.2 273.4 L 225.1 269.6 L 210.0 275.9 L 203.8 291.0 L 205.0 312.3 L 218.8 323.6 L 227.6 308.5 L 221.3 299.8"
    ],
    "tips": "Trace outline in 3 clean strokes for instant 90%+ AI recognition."
  },
  "boomerang": {
    "difficulty": "hard",
    "name": "Boomerang",
    "paths": [
      "M 46.3 283.5 L 46.3 268.4 L 95.2 124.1 L 105.3 106.5 L 130.4 80.2 L 165.5 70.1 L 280.9 73.9 L 348.7 92.7 L 356.2 99.0 L 360.0 135.4 L 351.2 141.6 L 317.3 145.4 L 278.4 145.4 L 204.4 132.9 L 163.0 135.4 L 147.9 163.0 L 115.3 247.1 L 94.0 283.5 L 58.8 321.1 L 42.5 329.9 L 40.0 292.2"
    ],
    "tips": "Trace aerodynamic curved throwing wing with rounded tips in one continuous stroke."
  },
  "bridge": {
    "difficulty": "hard",
    "name": "Bridge",
    "paths": [
      "M 60 300 L 60 140 L 340 140 L 340 300",
      "M 60 300 C 60 180 340 180 340 300"
    ],
    "tips": "1. Draw 3-sided rectangular bridge span. 2. Add curved arched opening underneath."
  },
  "broom": {
    "difficulty": "hard",
    "name": "Broom",
    "paths": [
      "M 174.3 230.7 L 127.8 329.9 L 141.6 312.3 L 158.0 263.4 L 179.3 220.7 L 154.2 360.0 L 189.3 252.1 L 191.8 229.5 L 189.3 214.4 L 189.3 252.1 L 194.4 274.7 L 223.2 333.6 L 235.8 351.2 L 188.1 278.4 L 183.1 255.8 L 183.1 218.2 L 204.4 249.6 L 263.4 297.3 L 272.2 309.8 L 233.3 277.2 L 210.7 249.6 L 189.3 211.9 L 189.3 76.4 L 185.6 40.0"
    ],
    "tips": "Trace broom handle and flared straw bristles in one continuous stroke."
  },
  "cat": {
    "difficulty": "hard",
    "name": "Cat",
    "paths": [
      "M 200 130 A 70 70 0 1 0 200 270 A 70 70 0 1 0 200 130 Z",
      "M 145 150 L 135 90 L 175 135",
      "M 255 150 L 265 90 L 225 135",
      "M 170 185 A 6 6 0 1 0 170 186",
      "M 230 185 A 6 6 0 1 0 230 186",
      "M 195 210 L 205 210 L 200 218 Z",
      "M 180 230 Q 200 245 220 230",
      "M 115 195 L 155 205",
      "M 115 220 L 155 215",
      "M 285 195 L 245 205",
      "M 285 220 L 245 215"
    ],
    "tips": "1. Draw round head with small triangular ears. 2. Add eyes, nose, smiley mouth, and whiskers."
  },
  "crab": {
    "difficulty": "hard",
    "name": "Crab",
    "paths": [
      "M 140 210 C 140 170 260 170 260 210 C 260 260 140 260 140 210 Z",
      "M 150 180 L 110 130 C 90 100 140 100 130 130",
      "M 110 130 C 80 150 130 160 130 140",
      "M 250 180 L 290 130 C 310 100 260 100 270 130",
      "M 290 130 C 320 150 270 160 270 140",
      "M 140 220 L 90 240",
      "M 140 240 L 90 270",
      "M 260 220 L 310 240",
      "M 260 240 L 310 270"
    ],
    "tips": "1. Draw oval shell body. 2. Add twin pinchers and crawling legs."
  },
  "dragon": {
    "difficulty": "hard",
    "name": "Dragon",
    "paths": [
      "M 250 100 L 290 100 L 270 130 L 230 140 L 210 220 L 140 260 L 90 290",
      "M 210 170 L 130 110 L 170 190",
      "M 130 110 L 90 160 L 150 180",
      "M 260 90 L 270 70 L 250 85",
      "M 160 260 L 160 320",
      "M 190 240 L 190 320"
    ],
    "tips": "1. Draw serpentine reptilian head and long neck. 2. Add large bat wings and long tail."
  },
  "elephant": {
    "difficulty": "hard",
    "name": "Elephant",
    "paths": [
      "M 240 200 C 240 130 130 130 130 200 L 130 310 L 160 310 L 160 250 L 200 250 L 200 310 L 230 310 L 230 200",
      "M 230 160 C 260 150 280 180 270 210 C 250 220 230 200 230 160",
      "M 270 170 C 290 170 310 200 300 250 C 300 270 320 270 320 250 C 325 190 290 150 265 150"
    ],
    "tips": "1. Draw massive body and thick pillar legs. 2. Add large floppy ear and curved trunk."
  },
  "flamingo": {
    "difficulty": "hard",
    "name": "Flamingo",
    "paths": [
      "M 180 250 L 180 360",
      "M 180 290 L 220 330",
      "M 150 200 C 120 200 110 240 140 250 C 180 260 230 255 240 215 C 240 190 200 190 150 200 Z",
      "M 225 205 C 255 180 255 120 235 100 C 220 85 240 60 265 60 C 280 60 290 70 290 85 L 310 95 L 285 105 C 275 130 270 180 235 210"
    ],
    "tips": "1. Draw tall standing legs. 2. Complete oval body and curved S-neck with beak."
  },
  "frog": {
    "difficulty": "hard",
    "name": "Frog",
    "paths": [
      "M 140 160 A 25 25 0 1 0 140 110 A 25 25 0 1 0 140 160 Z",
      "M 260 160 A 25 25 0 1 0 260 110 A 25 25 0 1 0 260 160 Z",
      "M 110 160 C 90 190 90 250 120 270 C 160 290 240 290 280 270 C 310 250 310 190 290 160 C 250 150 150 150 110 160 Z",
      "M 130 220 Q 200 260 270 220"
    ],
    "tips": "1. Draw twin bulging round eyes on top. 2. Add wide smiling frog head."
  },
  "garden": {
    "difficulty": "hard",
    "name": "Garden",
    "paths": [
      "M 60 300 L 340 300",
      "M 100 300 L 100 210",
      "M 100 190 A 20 20 0 1 0 100 150 A 20 20 0 1 0 100 190 Z",
      "M 200 300 L 200 190",
      "M 200 170 A 20 20 0 1 0 200 130 A 20 20 0 1 0 200 170 Z",
      "M 300 300 L 300 210",
      "M 300 190 A 20 20 0 1 0 300 150 A 20 20 0 1 0 300 190 Z"
    ],
    "tips": "1. Draw ground base line. 2. Add growing flower stems with circular blossoms."
  },
  "horse": {
    "difficulty": "hard",
    "name": "Horse",
    "paths": [
      "M 90 90 L 65 110 L 45 140 L 40 170 L 60 180 L 80 170 L 105 145 L 125 190 L 125 290 L 145 290 L 155 230 L 220 230 L 230 290 L 250 290 L 260 200 C 270 190 270 170 250 170 L 180 170 L 135 115 L 115 85 Z",
      "M 100 85 L 105 70 L 115 85",
      "M 60 140 L 65 145"
    ],
    "tips": "Trace complete horse profile: head, chest, legs, belly, and back without tail."
  },
  "lion": {
    "difficulty": "hard",
    "name": "Lion",
    "paths": [
      "M 227.0 166.7 L 219.5 147.9 L 208.2 136.6 L 199.4 130.4 L 184.3 126.6 L 164.2 126.6 L 141.6 135.4 L 126.6 149.2 L 120.3 164.2 L 117.8 216.9 L 121.6 229.5 L 139.1 245.8 L 161.7 258.4 L 198.1 260.9 L 227.0 243.3 L 239.5 224.5 L 244.5 204.4 L 244.5 168.0 L 233.3 137.9",
      "M 294.7 119.1 L 294.7 96.5 L 273.4 73.9 L 234.5 65.1 L 215.7 65.1 L 206.9 68.9 L 194.4 86.4 L 175.5 66.4 L 152.9 60.1 L 129.1 63.8 L 106.5 83.9 L 91.5 88.9 L 78.9 105.3 L 72.6 119.1 L 67.6 147.9 L 70.1 159.2 L 85.2 174.3 L 81.4 170.5 L 71.4 169.3 L 60.1 171.8 L 42.5 195.6 L 40.0 213.2 L 40.0 228.2 L 46.3 247.1 L 56.3 262.1 L 71.4 275.9 L 66.4 274.7 L 58.8 297.3 L 57.6 308.5 L 62.6 313.6 L 72.6 319.8 L 106.5 322.4 L 164.2 314.8 L 193.1 338.7 L 219.5 339.9 L 233.3 336.2 L 257.1 319.8 L 265.9 301.0 L 267.1 308.5 L 280.9 309.8 L 312.3 286.0 L 326.1 262.1 L 329.9 239.5 L 351.2 219.5 L 360.0 194.4 L 360.0 168.0 L 355.0 158.0 L 342.4 141.6 L 326.1 137.9 L 326.1 94.0 L 312.3 86.4 L 294.7 85.2"
    ],
    "tips": "Trace outline in 2 clean strokes for instant 90%+ AI recognition."
  },
  "map": {
    "difficulty": "hard",
    "name": "Map",
    "paths": [
      "M 48.8 99.0 L 41.3 165.5 L 42.5 299.8 L 173.0 306.0 L 283.5 329.9 L 331.1 334.9 L 353.7 327.4 L 360.0 282.2 L 360.0 189.3 L 350.0 65.1 L 313.6 76.4 L 238.3 87.7 L 73.9 86.4 L 42.5 87.7 L 40.0 90.2",
      "M 46.3 130.4 L 71.4 131.6 L 100.2 141.6 L 150.4 184.3 L 174.3 208.2 L 183.1 222.0 L 198.1 289.7 L 206.9 294.7 L 219.5 293.5 L 230.7 284.7 L 243.3 257.1 L 249.6 237.0 L 250.8 211.9 L 228.2 170.5 L 220.7 146.7 L 171.8 97.7 L 171.8 86.4",
      "M 100 190 L 115 205 L 130 190",
      "M 270 190 L 285 205 L 300 190"
    ],
    "tips": "1. Trace folded map border and internal road folds. 2. Add small 'v' terrain marks."
  },
  "monkey": {
    "difficulty": "hard",
    "name": "Monkey",
    "paths": [
      "M 175.4 108.0 L 161.6 118.1 L 152.8 132.0 L 150.2 183.6 L 160.3 198.7 L 196.9 215.1 L 238.4 216.4 L 267.4 203.8 L 280.0 188.7 L 286.3 173.5 L 287.6 144.6 L 277.5 125.7 L 259.8 111.8 L 223.3 103.0 L 183.0 104.3 L 176.7 108.0 L 179.2 111.8",
      "M 191.8 173.5 L 201.9 166.0",
      "M 257.3 163.5 L 257.3 163.5",
      "M 235.9 155.9 L 235.9 155.9",
      "M 238.4 178.6 L 261.1 177.3 L 263.6 168.5 L 257.3 163.5",
      "M 238.4 101.7 L 246.0 85.4 L 272.4 58.9 L 288.8 52.6 L 315.3 55.1 L 340.5 74.0 L 350.6 86.6 L 350.6 92.9 L 332.9 110.6 L 288.8 132.0",
      "M 206.9 99.2 L 206.9 86.6 L 200.6 71.5 L 190.6 57.6 L 178.0 47.6 L 160.3 41.3 L 132.6 40.0 L 116.2 48.8 L 103.6 71.5 L 104.9 101.7 L 130.1 123.1 L 167.9 142.0",
      "M 193.1 222.7 L 193.1 222.7",
      "M 185.5 213.9 L 179.2 251.7 L 147.7 273.1 L 137.6 314.6 L 118.7 329.8 L 98.6 328.5 L 72.1 308.3 L 49.4 300.8 L 49.4 347.4 L 52.0 352.4 L 62.0 360.0 L 135.1 360.0 L 142.7 356.2"
    ],
    "tips": "Trace outline in 9 clean strokes for instant 90%+ AI recognition."
  },
  "octopus": {
    "difficulty": "hard",
    "name": "Octopus",
    "paths": [
      "M 140 180 C 130 100 270 100 260 180 C 260 220 140 220 140 180 Z",
      "M 175 170 A 5 5 0 1 0 175 171",
      "M 225 170 A 5 5 0 1 0 225 171",
      "M 150 220 Q 130 290 100 310",
      "M 170 220 Q 160 290 140 320",
      "M 190 220 Q 190 300 180 330",
      "M 210 220 Q 210 300 220 330",
      "M 230 220 Q 240 290 260 320",
      "M 250 220 Q 270 290 300 310"
    ],
    "tips": "1. Draw large bulbous mantle head. 2. Add twin eyes. 3. Radiate eight wavy curling tentacles below."
  },
  "owl": {
    "difficulty": "hard",
    "name": "Owl",
    "paths": [
      "M 130 130 L 140 80 L 170 110 L 230 110 L 260 80 L 270 130 C 290 200 280 290 250 320 L 150 320 C 120 290 110 200 130 130 Z",
      "M 165 170 A 25 25 0 1 0 165 171",
      "M 235 170 A 25 25 0 1 0 235 171",
      "M 190 195 L 210 195 L 200 215 Z"
    ],
    "tips": "1. Draw tufted owl body outline. 2. Add big round ring eyes and triangular beak."
  },
  "parrot": {
    "difficulty": "hard",
    "name": "Parrot",
    "paths": [
      "M 210 110 C 190 90 160 110 170 140 L 170 260 L 150 340 L 175 340 L 195 270 C 220 250 220 180 210 110 Z",
      "M 200 115 C 240 120 250 150 220 165 C 205 165 200 140 200 115 Z",
      "M 185 125 A 4 4 0 1 0 185 126"
    ],
    "tips": "1. Draw round head with large curved hooked beak. 2. Add perch body with long tail feathers."
  },
  "penguin": {
    "difficulty": "hard",
    "name": "Penguin",
    "paths": [
      "M 200 90 C 160 90 150 140 150 200 C 140 240 150 310 200 310 C 250 310 260 240 250 200 C 250 140 240 90 200 90 Z",
      "M 170 200 C 170 290 230 290 230 200 C 230 150 170 150 170 200 Z",
      "M 190 120 L 210 120 L 200 135 Z",
      "M 150 180 C 130 210 130 250 145 270",
      "M 250 180 C 270 210 270 250 255 270",
      "M 170 310 L 160 330 L 190 330 Z",
      "M 230 310 L 240 330 L 210 330 Z"
    ],
    "tips": "1. Draw bowling-pin shaped body with white belly arch. 2. Add beak, flipper wings, and waddling feet."
  },
  "pickup truck": {
    "difficulty": "hard",
    "name": "Pickup Truck",
    "paths": [
      "M 70 230 L 70 180 L 190 180 L 190 130 L 280 130 L 320 180 L 320 230 Z",
      "M 190 180 L 190 230",
      "M 110 230 A 22 22 0 1 0 154 230",
      "M 240 230 A 22 22 0 1 0 284 230"
    ],
    "tips": "1. Draw cab with windshield. 2. Add open rectangular cargo bed behind. 3. Add two wheels."
  },
  "purse": {
    "difficulty": "hard",
    "name": "Purse",
    "paths": [
      "M 125 180 L 275 180 L 310 260 C 310 280 90 280 90 260 Z",
      "M 162 180 C 162 60 238 60 238 180"
    ],
    "tips": "1. Draw handbag body with curved base. 2. Add high arched handle above."
  },
  "rabbit": {
    "difficulty": "hard",
    "name": "Rabbit",
    "paths": [
      "M 200 160 A 35 35 0 1 0 200 230 A 35 35 0 1 0 200 160 Z",
      "M 180 165 C 160 110 160 60 180 60 C 200 60 200 110 185 165",
      "M 215 165 C 200 110 200 60 220 60 C 240 60 240 110 220 165",
      "M 195 205 L 205 205 L 200 215 Z",
      "M 175 225 C 150 250 150 300 200 310 C 240 310 245 280 230 225",
      "M 145 290 A 10 10 0 1 0 145 291"
    ],
    "tips": "1. Draw round head with very tall upright oval ears. 2. Add twitchy nose and fluffy tail."
  },
  "sea turtle": {
    "difficulty": "hard",
    "name": "Sea Turtle",
    "paths": [
      "M 200 120 C 150 120 140 180 140 240 C 140 290 170 310 200 310 C 230 310 260 290 260 240 C 260 180 250 120 200 120 Z",
      "M 200 120 C 190 80 210 80 200 120",
      "M 150 150 C 90 120 80 170 140 200",
      "M 250 150 C 310 120 320 170 260 200",
      "M 155 280 L 130 320",
      "M 245 280 L 270 320"
    ],
    "tips": "1. Draw oval shell. 2. Add head, large front swimming flippers, and rear flippers."
  },
  "sheep": {
    "difficulty": "hard",
    "name": "Sheep",
    "paths": [
      "M 140 180 C 130 140 170 120 190 135 C 220 120 250 130 265 150 C 290 145 315 170 305 200 C 320 230 300 265 270 260 C 250 275 210 275 190 260 C 160 270 130 250 140 220 C 120 210 120 190 140 180 Z",
      "M 100 170 C 80 150 75 200 95 210 C 120 215 130 180 100 170 Z",
      "M 85 170 C 65 175 70 195 85 185",
      "M 110 170 C 130 175 125 195 110 185",
      "M 160 265 L 160 320",
      "M 185 265 L 185 320",
      "M 245 265 L 245 320",
      "M 270 265 L 270 320"
    ],
    "tips": "1. Draw cloud-like fluffy fleece body. 2. Add head with drooping ears. 3. Add four vertical legs."
  },
  "spider": {
    "difficulty": "hard",
    "name": "Spider",
    "paths": [
      "M 200 150 A 45 45 0 1 0 200 240 A 45 45 0 1 0 200 150 Z",
      "M 70 140 L 125 170 L 165 185",
      "M 70 195 L 125 195 L 165 195",
      "M 70 250 L 125 220 L 165 205",
      "M 330 140 L 275 170 L 235 185",
      "M 330 195 L 275 195 L 235 195",
      "M 330 250 L 275 220 L 235 205"
    ],
    "tips": "1. Draw central round spider body. 2. Add six jointed legs (three on each side)."
  },
  "squirrel": {
    "difficulty": "hard",
    "name": "Squirrel",
    "paths": [
      "M 200 140 A 20 20 0 1 0 200 180 A 20 20 0 1 0 200 140 Z",
      "M 195 130 L 190 110 L 205 125",
      "M 180 180 C 160 220 160 270 190 290 C 220 290 230 250 220 200",
      "M 210 200 L 230 200",
      "M 180 290 C 130 280 110 160 140 100 C 160 70 190 80 180 120 C 165 150 155 220 190 260"
    ],
    "tips": "1. Draw rodent body sitting up holding nut. 2. Add huge bushy S-curled tail arched over back."
  },
  "submarine": {
    "difficulty": "hard",
    "name": "Submarine",
    "paths": [
      "M 80 200 C 80 160 300 160 330 200 C 300 240 80 240 80 200 Z",
      "M 180 160 L 180 120 L 220 120 L 220 160",
      "M 200 120 L 200 90 L 215 90",
      "M 80 200 L 60 180 L 60 220 Z"
    ],
    "tips": "1. Draw elongated cylindrical sub hull. 2. Add conning tower periscope on top. 3. Add propeller at rear."
  },
  "swan": {
    "difficulty": "hard",
    "name": "Swan",
    "paths": [
      "M 90 115 L 75 120 L 95 125",
      "M 90 115 C 90 80 135 80 135 110 C 135 140 100 190 90 240 C 85 270 110 310 160 310 L 290 310 C 320 310 325 250 330 230 C 300 245 250 230 210 230 C 160 230 125 250 115 210 C 105 170 120 140 105 130 L 95 125 Z",
      "M 160 240 C 210 225 270 230 300 240 C 270 260 200 260 160 240"
    ],
    "tips": "1. Draw small beak pointing left. 2. Trace graceful S-neck, breast, flat bottom, and pointed tail. 3. Add wing curve."
  }
};

export function getBlueprintForClass(className) {
  const key = className ? className.toLowerCase().trim() : '';
  if (CURATED_BLUEPRINTS[key]) {
    return CURATED_BLUEPRINTS[key];
  }

  // Find class display info
  const item = ACTIVE_CLASSES_ORDERED.find((c) => c.name?.toLowerCase() === key || c.class_name?.toLowerCase() === key);
  const diff = item ? (item.difficulty || item.label) : 'medium';
  const display = item ? (item.name || item.class_name) : className;

  return {
    name: display,
    difficulty: diff,
    tips: `Trace the outline clearly for ${display}. Keep strokes steady and continuous.`,
    paths: []
  };
}

export function getAllLearningLessons() {
  return ACTIVE_CLASSES_ORDERED.map((item, idx) => {
    const cname = item.class_name || item.name;
    const bp = getBlueprintForClass(cname);
    return {
      level: idx + 1,
      id: cname,
      className: cname,
      displayName: bp.name || cname,
      difficulty: item.label || item.difficulty || 'medium',
      tips: bp.tips,
      paths: bp.paths || []
    };
  });
}
